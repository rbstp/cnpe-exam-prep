...
The study pages use a reading-first black-and-gold theme, dark by default with a full light theme (`t`). **Read / Practice / Recall** and the exercise index jump directly through the original continuous lesson; **Contents** provides the same access on mobile. Use `f` for focus view and **Aa** for larger reading text. Text size and theme stay local to the browser, outside progress sync. The quest retains its original layout and system/light/dark theme switch.
