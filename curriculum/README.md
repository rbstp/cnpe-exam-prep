...
- `tools/subset-fonts.py`: re-cuts `assets/fonts/` from the untouched latin builds in `tools/fonts-src/` down to the characters the pages, scripts and stylesheet actually use, which is 109 of the 229 those builds carry: 126 KB of faces becomes 80 KB, twice over, since the bundle inlines them again. It needs `fonttools` and `brotli`, so the result is committed rather than built; `--check` needs neither and is what CI runs, failing when new prose reaches for a character the cut removed (the codepoints it removed are listed in `tools/fonts-src/dropped.txt`). Run `make fonts` and commit `assets/fonts/` when it fires
