...
- `assets/`: one stylesheet, the section manifest (`nav.js`), the progress merge (`merge.js`), the page runtime (`app.js`), the interactive figures (`widgets.js`), the drill (`drill.js`), the optional progress sync (`sync.js`), the theme switch (`theme.js`, loaded from `<head>`) and the typefaces
- `assets/fonts/`: IBM Plex Sans, Sans Condensed and Mono (latin subsets), shipped so the console needs no network; the sans is one variable file covering wght 100-700, the other four are static weights; SIL OFL 1.1, see `OFL.txt`
...
- `tools/merge-test.mjs`: drives the progress merge over plain objects: the whole base/local/remote truth table, the counter maxima and the guards on a payload that arrives over the network (`node tools/merge-test.mjs`, or `make merge`); no dependencies, no browser, and CI runs it on every PR
