...
(function (root) {
  ...
  /** the tool family, for the prompt and the cheat sheets @param {string} cmd */
  function toolOf(cmd) {
    var n = normalize(cmd);
  ...
  /** singular, for kubectl's messages @param {string} kind */
  ...
  /** @param {CnpeGameResource} r @param {string} kind */
  function fullName(kind, r) { return singular(kind) + "/" + r.name; }

  /**
   * The resources a command names, or null when the kind is unknown here.
   * @param {CnpeGameScenario} sc @param {string} kind @param {string} [name]
   * @param {string} [ns] @param {boolean} [all] @param {string} [sel]
   */
  ...
  /** @param {string} kind @param {string} ns */
  ...
  /** two-space yaml of a resource, from what the table knows
      @param {string} kind @param {CnpeGameResource} r */
  ...
  /** @param {CnpeGameScenario} sc @param {string} kind @param {CnpeGameResource} r */
  ...
  /** a jsonpath-ish lookup over what the resource declares
      @param {CnpeGameResource} r @param {string} path */
  ...
  /** the reply to a get, in whatever output the flags ask for
      @param {CnpeGameScenario} sc @param {string} kind @param {CnpeGameResource[]} res @param {{[k: string]: string}} f @param {boolean} many */
   ...
   * @param {CnpeGameScenario} sc
   * @param {Record<string, number>} found evidence ids surfaced so far
   * @param {string} cmd
   * @return {CnpeSimResult}
   */
  function run(sc, found, cmd) {
    ...
    if (hit) {
      ...
      var r = /** @type {CnpeSimResult} */ ({ out: out });
  ...
  /** @param {string} n normalised command @return {{tool: string, pos: string[], f: {[k: string]: string}}} */
  function parts(n) {
  ...
  /** @param {CnpeGameScenario} sc @param {string} n */
  ...
  /** @param {CnpeGameScenario} sc @param {string[]} pos @param {{[k: string]: string}} f @param {string} ns @param {string} verb */
  ...
  /** @param {CnpeGameScenario} sc @param {{[k: string]: string}} f @param {string} ns */
  ...
  /** @param {CnpeGameScenario} sc @param {string[]} pos @param {{[k: string]: string}} f @param {string} ns */
  ...
  /** @param {CnpeGameResource} r @param {{[k: string]: string}} f */
  ...
  /** @param {CnpeGameScenario} sc @param {string[]} pos @param {{[k: string]: string}} f @param {string} verb */
  ...
  function flux(sc, pos, f, ns, verb) {
    ...
    if (verb === "get") {
      if (kind === "sources") { kind = ({ git: "gitrepositories", oci: "ocirepositories", helm: "helmrepositories" })[pos[2]] || ""; name = pos[3]; }
  ...
  function argocd(sc, pos, f, verb) {
    ...
    if (sub === "list") {
      ...
      return table(["NAME", "CLUSTER", "NAMESPACE", "PROJECT", "STATUS", "HEALTH", "SYNCPOLICY", "CONDITIONS", "REPO", "PATH", "TARGET"],
        apps.map(function (a) { var g = a.argo || {}; return ["argocd/" + a.name, "https://kubernetes.default.svc", g.dest || "", "default", g.sync || (a.cols || [])[0] || "", g.health || (a.cols || [])[1] || "", "Auto-Prune", g.conditions || "<none>", g.repo || "", g.path || "", g.rev || ""]; }));
    }
  ...
  function rollouts(sc, pos, f, ns, verb) {
    ...
    if (verb === "list") return table(["NAME", "STRATEGY", "STATUS", "STEP", "SET-WEIGHT", "READY", "DESIRED", "UP-TO-DATE", "AVAILABLE"], (find(sc, "rollouts", "", ns) || []).map(function (r) { var g = /** @type {*} */ (r.rollout || {}); return [r.name, "Canary", g.status || "", g.step || "", g.weight || "", "2/2", "2", "2", "2"]; }));
    if (!name) return "Error: accepts 1 arg(s), received 0";
    var res = find(sc, "rollouts", name, ns);
    if (!res || !res.length) return 'Error: rollouts.argoproj.io "' + name + '" not found';
    var r = res[0], g = /** @type {*} */ (r.rollout || {});
