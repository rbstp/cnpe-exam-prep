...
(function (root) {
    ...
    var CLUSTER = { namespaces: 1, nodes: 1, storageclasses: 1, persistentvolumes: 1, clusterroles: 1,
    ...
    function kindOf(k) {
        ...
        if (KINDS[low])
            return KINDS[low];
        if (dot > 0 && KINDS[low.slice(0, dot)])
    ...
    function kindTok(tok) {
        return tok.split(",").map(function (part) {
            ...
            if (slash > 0)
                return kindOf(part.slice(0, slash)) + " " + part.slice(slash + 1);
            return kindOf(part);
        }).join(",");
    }
    /** shell-ish split: quotes group, and the first pipe or chain ends the command */
    function tokens(s) {
        var out = [], cur = "", q = "", i, c, had = false;
    ...
    var BARE = { "-A": "-A", "--all-namespaces": "-A", "--previous": "--previous", "--show-labels": "--show-labels",
    ...
    var SHORT_BARE = { A: "-A", w: "-w", i: "-i", t: "-t", h: "--help" };
    /**
    ...
    function normalize(cmd) {
        ...
        var pos = [], flags = {}, verb = "", i, tok, m, name, val, rest = null;
        for (i = 1; i < t.length; i++) {
            ...
            if (tok[0] === "-" && tok.length > 1 && !/^-\d/.test(tok)) {
                ...
                name = m[1].length > 2 ? m[1].toLowerCase() : m[1];
                val = m[2];
                // -p is --previous for logs and --patch for patch; decide by the verb so far
                if (name === "-p")
                    name = (verb === "logs" || (pos[0] === "logs")) ? "--previous" : "--patch";
                // and -f is --follow there, not --filename
                if (name === "-f" && val === undefined && (verb === "logs" || pos[0] === "logs")) {
                    flags["-f"] = true;
                    continue;
                }
                if (name in BARE && val === undefined) {
                    flags[BARE[name]] = true;
                    continue;
                }
                if (name in VALUED) {
                    if (val === undefined)
                        val = i + 1 < t.length ? t[++i] : "";
                    if (VALUED[name])
                        flags[VALUED[name]] = val;
                    continue;
                }
                // -nteam-a, -oyaml, -ltier=web: a short flag glued to its value
                m = /^-([nolcf])(.+)$/.exec(tok);
                if (m) {
                    flags[VALUED["-" + m[1]]] = m[2];
                    continue;
                }
                // -itp and friends
            ...
            pos.push(tok);
            if (!verb)
                verb = tok.toLowerCase();
        }
        // kubectl argo rollouts ... is its own tool
        if (tool === "kubectl" && pos[0] === "argo" && pos[1] === "rollouts") {
            tool = "kubectl argo rollouts";
            pos = pos.slice(2);
        }
        if (tool === "crossplane" && pos[0] === "beta") {
            pos.shift();
            tool = "crossplane beta";
        }
        ...
        var kindAt = -1;
        if (tool === "kubectl") {
            if (["get", "describe", "delete", "edit", "patch", "label", "annotate", "scale", "explain", "expose", "top", "wait", "create"].indexOf(verb) >= 0)
                kindAt = 1;
            else if (verb === "set" || verb === "rollout")
                kindAt = 2;
            else if (verb === "logs" || verb === "exec" || verb === "port-forward")
                kindAt = 1;
            else if (verb === "auth" && pos[1] === "can-i")
                kindAt = 3;
        }
        else if (tool === "flux") {
            if (["get", "resume", "suspend", "reconcile", "tree", "delete", "export", "create", "logs"].indexOf(verb) >= 0)
                kindAt = 1;
            if (verb === "get" && pos[1] && /^sources?$/.test(pos[1].toLowerCase())) {
                pos[1] = "sources";
                kindAt = -1;
            }
        }
            ...
            if (pos[0])
                pos[0] = kindOf(pos[0]);
            if (pos[1])
                pos[1] = pos[1].toLowerCase();
        }
        else if (tool === "crossplane beta" || tool === "kubectl argo rollouts") {
            if (["trace", "get", "retry", "promote", "abort", "undo", "restart", "status"].indexOf(verb) >= 0)
                kindAt = 1;
        }
        else if (tool === "argocd") {
            if (pos[1])
        ...
        if (tool === "kubectl" && (verb === "logs" || verb === "exec" || verb === "port-forward") && pos[1] === "pods" && pos[2])
            pos.splice(1, 1);
        if (verb === "auth" && pos[1] === "can-i" && pos[2])
            pos[2] = pos[2].toLowerCase();
        if (verb === "set" && pos[1])
            pos[1] = pos[1].toLowerCase();
        if (verb === "rollout" && pos[1])
            pos[1] = pos[1].toLowerCase();
        ...
        if (typeof flags["-p"] === "string")
            flags["-p"] = flags["-p"].replace(/\s+/g, "").toLowerCase();
        if (typeof flags["-o"] === "string")
            flags["-o"] = flags["-o"].toLowerCase();
        if (typeof flags["--type"] === "string")
            flags["--type"] = flags["--type"].toLowerCase();
    ...
    function toolOf(cmd) {
        var n = normalize(cmd);
        if (/^kubectl argo rollouts\b/.test(n) || /^argocd\b/.test(n))
            return "argo";
        if (/^kubectl get (analysisruns|analysistemplates|rollouts|applications|workflows)\b/.test(n))
            return "argo";
        if (/^flux\b/.test(n) || /^kubectl \S+ (kustomizations|gitrepositories|helmreleases)\b/.test(n))
            return "flux";
        if (/^tkn\b/.test(n) || /^kubectl \S+ (pipelineruns|pipelines|tasks|taskruns|eventlisteners|triggerbindings|triggertemplates)\b/.test(n))
            return "tekton";
        if (/^crossplane/.test(n) || /^kubectl \S+ (appenvironments|objects|compositions|compositeresourcedefinitions|providerconfigs)\b/.test(n))
            return "crossplane";
        if (/^kubectl \S+ (servicemonitors|podmonitors|prometheusrules|prometheuses|opentelemetrycollectors|validatingpolicies|imagevalidatingpolicies|clusterpolicies|policyreports|externalsecrets|secretstores|clustersecretstores)\b/.test(n))
            return "platform";
        if (/^kubectl\b/.test(n))
            return "kubectl";
        return n.split(" ")[0] || "";
    ...
    var EVENT_COLS = ["LAST SEEN", "TYPE", "REASON", "OBJECT", "MESSAGE"];
    function table(head, rows) {
        var w = head.map(function (h) { return h.length; });
        rows.forEach(function (r) { r.forEach(function (c, i) { if (String(c).length > (w[i] || 0))
            w[i] = String(c).length; }); });
        function line(r) {
    ...
    function singular(kind) {
        if (/ies$/.test(kind))
            return kind.replace(/ies$/, "y");
        if (/(ss|us)es$/.test(kind))
            return kind.replace(/es$/, "");
        return kind.replace(/s$/, "");
    }
    function fullName(kind, r) { return singular(kind) + "/" + r.name; }
    /**
     * The resources a command names, or null when the kind is unknown here.
     */
    function find(sc, kind, name, ns, all, sel) {
        var res = sc.resources.filter(function (r) { return r.kind === kind; });
        if (!(kind in COLS) && !res.length)
            return null;
        if (!CLUSTER[kind] && !all)
            res = res.filter(function (r) { return (r.ns || "default") === ns; });
        if (name)
            res = res.filter(function (r) { return r.name === name; });
        if (sel) {
            var wants = sel.split(",");
    ...
    function describe(sc, kind, r) {
        ...
        if (!evs.length)
            ...
            lines.push("Events:");
            lines.push("  " + table(["Type", "Reason", "Age", "From", "Message"], evs.map(function (e) { return [e.type, e.reason, e.age, e.from, e.msg]; })).split("\n").join("\n  "));
        }
        return lines.join("\n");
    }
    /** a jsonpath-ish lookup over what the resource declares */
    function jsonpath(r, path) {
        var p = path.replace(/^jsonpath=/, "").replace(/^\{|\}$/g, "").trim();
        if (r.fields && p in r.fields)
            return r.fields[p];
        if (p === ".metadata.name")
            return r.name;
    ...
    function renderGet(sc, kind, res, f, many) {
        ...
        if (/^yaml$/.test(o) || /^json$/.test(o)) {
            ...
            if (o === "json")
                return "(json output is long; try -o yaml, or -o jsonpath=...)\n" + docs.join("\n---\n");
        ...
        if (o && o !== "wide" && o !== "name")
            return 'error: unable to match a printer suitable for the output format "' + o + '", allowed formats are: wide,yaml,json,jsonpath';
        if (o === "name")
            return res.map(function (r) { return fullName(kind, r); }).join("\n");
        var head = ["NAME"].concat(COLS[kind] || []);
        if (f["-A"] && !CLUSTER[kind])
            head.unshift("NAMESPACE");
        if (o === "wide" && res.some(function (r) { return r.wide; }))
            head = head.concat(res[0].wideCols || ["NODE"]);
        if (f["--show-labels"])
            head.push("LABELS");
        var rows = res.map(function (r) {
            var row = [many ? fullName(kind, r) : r.name].concat(r.cols || []);
            while (row.length < 1 + (COLS[kind] || []).length)
                row.push("");
            if (f["-A"] && !CLUSTER[kind])
                row.unshift(r.ns || "default");
            if (o === "wide" && head.length > row.length)
                row = row.concat(r.wide || []);
            if (f["--show-labels"])
                row.push(r.labels || "<none>");
            return row;
        });
     ...
     */
    function run(sc, found, cmd) {
        var n = normalize(cmd);
        if (!n)
            return { out: "", generic: true };
        var hit = null, i;
    ...
    function parts(n) {
        ...
        for (; i < t.length; i++) {
            ...
            if (tok[0] === "-" && tok.length > 1 && !/^-\d/.test(tok)) {
                var eq = tok.indexOf("=");
                if (eq > 0)
                    f[tok.slice(0, eq)] = tok.slice(eq + 1);
                else if ((tok === "-n" || tok === "-l" || tok === "-o" || tok === "-c" || tok === "-f") && i + 1 < t.length)
                    f[tok] = t[++i];
                else
                    f[tok] = "true";
            }
    ...
    function generic(sc, n) {
        var p = parts(n), tool = p.tool, pos = p.pos, f = p.f, verb = pos[0] || "";
        var ns = f["-n"] || "default";
        if (f["--help"])
            return HELP[tool] || ("bash: " + tool + ": command not found");
        if (tool === "kubectl")
            return kubectl(sc, pos, f, ns, verb);
        if (tool === "flux")
        ...
        if (tool === "curl" || tool === "wget")
            return "curl: (7) Failed to connect: the terminal runs outside the cluster; kubectl exec into a pod to probe from inside";
        if (tool === "ls" || tool === "cat" || tool === "cd" || tool === "vim" || tool === "vi" || tool === "nano")
            return "bash: " + tool + ": this terminal has no filesystem; every repair is a kubectl, flux, argocd or tkn command";
        if (tool === "make")
            return "make: *** No rule to make target here; the lab's make targets run on your machine, not in the quest";
        return "bash: " + tool + ": command not found";
    }
    function kubectl(sc, pos, f, ns, verb) {
        var kind, name, res;
        switch (verb) {
            case "":
                return HELP.kubectl;
            case "get":
                if (!pos[1])
                    return "error: You must specify the type of resource to get. Use \"kubectl api-resources\" for a complete list of supported resources.";
                if (pos[1] === "events")
                    return getEvents(sc, f, ns);
                if (pos[1] === "all") {
                    var all = ["pods", "services", "deployments", "replicasets", "horizontalpodautoscalers", "rollouts"], blocks = [];
            ...
            case "top":
                ...
                if (kind === "pods") {
                    ...
                    if (!res || !res.length)
                        return "error: Metrics not available for pods in " + ns + " namespace";
                    return table(["NAME", "CPU(cores)", "MEMORY(bytes)"], res.map(function (r) { return [r.name].concat(r.top || ["1m", "6Mi"]); }));
                }
                if (kind === "nodes")
                    return table(["NAME", "CPU(cores)", "CPU%", "MEMORY(bytes)", "MEMORY%"], [["cnpe-control-plane", "412m", "10%", "2611Mi", "33%"], ["cnpe-worker", "388m", "9%", "2210Mi", "28%"], ["cnpe-worker2", "351m", "8%", "1980Mi", "25%"]]);
                return "error: unknown command \"" + kind + "\" for \"kubectl top\"";
            case "config":
                if (pos[1] === "get-contexts")
                    return "CURRENT   NAME        CLUSTER     AUTHINFO    NAMESPACE\n*         kind-cnpe   kind-cnpe   kind-cnpe";
                if (pos[1] === "current-context")
                    return "kind-cnpe";
    ...
    function logs(sc, pos, f, ns) {
        var kind = "pods", name = pos[1], res;
        if (!name)
            return "error: expected 'logs [-f] [-p] (POD | TYPE/NAME) [-c CONTAINER]'.\nPOD or TYPE/NAME is a required argument for the logs command";
        if (KINDS[name] && pos[2]) {
            kind = KINDS[name];
            name = pos[2];
        }
        if (kind === "pods") {
            ...
            if (!res.length && !f["-l"]) {
                ...
                if (!owner.length)
                    return 'error: error from server (NotFound): pods "' + name + '" not found in namespace "' + ns + '"';
                kind = "deployments";
            }
        }
        if (kind !== "pods") {
            ...
            if (!pods.length)
                return "error: no pods found for " + singular(kind) + "/" + name + " in " + ns;
            res = [pods[0]];
            if (pods.length > 1)
                return "Found " + pods.length + " pods, using pod/" + pods[0].name + "\n" + podLogs(pods[0], f);
        }
    ...
    function canI(sc, pos, f, ns) {
        if (pos[1] !== "can-i")
            return 'error: unknown command "' + (pos[1] || "") + '" for "kubectl auth"';
        var who = f["--as"] || "";
        var table_ = sc.canI || {};
        if (f["--list"]) {
            if (!who || !table_[who])
        ...
        if (!verb || !res)
            return "error: you must specify two or three arguments: verb, resource, and optional resourceName";
        if (!who)
            return "yes";
        var mine = table_[who];
        if (!mine)
    ...
    function exec(sc, pos, f, ns) {
        ...
        var r = res[0], cmd = f["--"] || "";
        if (r.notRunning)
            return 'error: unable to upgrade connection: container not found ("' + (r.container || "app") + '")';
        if (!cmd)
            return "error: you must specify at least one command for the container";
        if (r.exec) {
            for (var k in r.exec)
                if (new RegExp(k).test(cmd))
                    return r.exec[k];
        }
        if (/^(sh|bash|ash)\b/.test(cmd) && !/-c/.test(cmd))
            return "(an interactive shell does not run in the quest's terminal; pass the command after --, as in -- sh -c 'nslookup kubernetes.default')";
        if (/nslookup|dig|getent|host\b/.test(cmd))
            return "Server:    10.96.0.10\nAddress:   10.96.0.10:53\n\nName:      kubernetes.default.svc.cluster.local\nAddress:   10.96.0.1";
        if (/wget|curl/.test(cmd))
            return "Connecting to " + (/(\S+:\/\/\S+|\S+\.svc\S*)/.exec(cmd) || ["the address"])[0] + "\nHTTP/1.1 200 OK";
        if (/^(ls|cat|env|printenv|ps|id|whoami|hostname|date|uname|df|mount)\b/.test(cmd))
            return "(" + cmd + ": runs, and shows nothing the fault touches)";
        return "sh: " + cmd.split(" ")[0] + ": not found";
    }
    function mutate(sc, pos, f, ns, verb) {
        var kind, name, res;
        if (verb === "edit")
            return "error: the quest has no editor. Use patch, set, scale, label, annotate or delete for the same edit.";
        if (verb === "apply" || verb === "replace") {
            if (f["-f"] === "-")
                return "error: no stdin in the quest's terminal; write the change as a patch";
        ...
        if (verb === "create") {
            if (f["-f"])
                return 'error: the path "' + f["-f"] + '" does not exist (the terminal has no filesystem)';
            if (!pos[1])
                return "error: You must specify the type of resource to create.";
            if (!pos[2])
                return "error: exactly one NAME is required, got 0";
            kind = pos[1];
            name = pos[2];
            res = find(sc, kind, name, ns);
            if (res && res.length)
                return 'error: failed to create ' + singular(kind) + ': ' + singular(kind) + 's "' + name + '" already exists';
        ...
        if (verb === "set") {
            ...
            if (!res || !res.length)
                return notFound(kind, name);
            if (what === "image")
                return (pos[4] ? singular(kind) + "/" + name + " image updated" : "error: at least one image update is required");
            if (what === "resources")
                return singular(kind) + "/" + name + " resource requirements updated";
            if (what === "env")
                return singular(kind) + "/" + name + " env updated";
            if (what === "serviceaccount")
                return singular(kind) + "/" + name + " serviceaccount updated";
            return 'error: unknown command "' + what + '" for "kubectl set"';
        }
        kind = pos[1];
        name = pos[2];
        if (!kind)
            return "error: You must specify the type of resource.";
        res = find(sc, kind, name, ns, false, f["-l"]);
        if (res === null)
            return 'error: the server doesn\'t have a resource type "' + kind + '"';
        if (!name && !f["-l"] && !f["--all"])
        ...
        if (!res.length)
            return name ? notFound(kind, name) : none(kind, ns);
        var target = res.map(function (r) { return fullName(kind, r); }).join("\n");
        switch (verb) {
            case "patch":
                if (!f["-p"])
                    return "error: must specify -p to patch";
                if (f["--type"] === "json" && !/^\[/.test(f["-p"]))
                    return "error: unable to parse \"" + f["-p"] + "\": a json patch is a list of operations";
    ...
    function flux(sc, pos, f, ns, verb) {
        ...
        if (verb === "get") {
            ...
            if (!res || !res.length)
                return "no " + kind + " objects found in " + ns + " namespace";
        ...
        if (verb === "resume" || verb === "suspend" || verb === "reconcile" || verb === "tree") {
            if (!kind || !name)
                return "error: " + verb + " needs a kind and a name, as in flux " + verb + " kustomization <name>";
            var one = find(sc, kind, name, ns);
            if (!one || !one.length)
                return "✕ " + kindName(kind) + " object '" + name + "' not found in \"" + ns + "\" namespace";
            if (verb === "tree")
                return kindName(kind) + "/" + ns + "/" + name + "\n" + (one[0].tree || "└── (nothing applied yet)");
            if (verb === "reconcile")
                return "► annotating " + kindName(kind) + " " + name + " in " + ns + " namespace\n✔ " + kindName(kind) + " annotated\n◎ waiting for " + kindName(kind) + " reconciliation\n" + (one[0].reconcile || "✔ " + kindName(kind) + " reconciliation completed\n✔ applied revision " + ((one[0].flux || [])[0] || "main"));
        ...
        if (verb === "logs")
            return "(flux logs stream from the controllers; the objects' READY column and MESSAGE say the same thing sooner)";
        if (verb === "check")
    ...
    function argocd(sc, pos, f, verb) {
        ...
        if (sub === "list") {
            if (!apps.length)
                return "NAME  CLUSTER  NAMESPACE  PROJECT  STATUS  HEALTH  SYNCPOLICY  CONDITIONS  REPO  PATH  TARGET";
            return table(["NAME", "CLUSTER", "NAMESPACE", "PROJECT", "STATUS", "HEALTH", "SYNCPOLICY", "CONDITIONS", "REPO", "PATH", "TARGET"], apps.map(function (a) { var g = a.argo || {}; return ["argocd/" + a.name, "https://kubernetes.default.svc", g.dest || "", "default", g.sync || (a.cols || [])[0] || "", g.health || (a.cols || [])[1] || "", "Auto-Prune", g.conditions || "<none>", g.repo || "", g.path || "", g.rev || ""]; }));
        }
        if (!name)
            return "Error: accepts 1 arg(s), received 0";
        var app = apps.filter(function (a) { return a.name === name; })[0];
        if (!app)
            return 'FATA[0000] rpc error: code = NotFound desc = applications.argoproj.io "' + name + '" not found';
        var g = app.argo || {};
        if (sub === "get") {
        ...
        if (sub === "sync")
            return g.syncOut || ("TIMESTAMP                  GROUP  KIND  NAMESPACE  NAME  STATUS  HEALTH  HOOK  MESSAGE\n" + new Date().toISOString().slice(0, 19) + "  apps   Deployment  " + (g.dest || "") + "  " + app.name + "  Synced  Healthy");
        if (sub === "diff")
            return g.diff || "(no differences between the live state and the target revision)";
        if (sub === "set")
            return "application '" + app.name + "' updated\n(the field changed, and the compare still fails: that was not the one)";
        if (sub === "history")
            return "ID  DATE                           REVISION\n1   " + new Date().toISOString().slice(0, 19) + "  main (a1b2c3d)";
        if (sub === "manifests" || sub === "resources")
    ...
    function tkn(sc, pos, f, ns) {
        ...
        if (!kind)
            return HELP.tkn;
        if (!(kind in COLS))
            return 'Error: unknown command "' + kind + '" for "tkn"';
        var res = find(sc, kind, sub === "list" ? "" : name, ns);
        if (sub === "list") {
            if (!res || !res.length)
                return "No " + kindName(kind) + "s found";
            if (kind === "pipelineruns")
                return table(["NAME", "STARTED", "DURATION", "STATUS"], res.map(function (r) { var c = r.cols || []; return [r.name, c[2] || "", c[3] && c[3] !== "---" ? "2s" : "---", (c[0] === "True" ? "Succeeded" : c[0] === "False" ? "Failed(" + (c[1] || "") + ")" : "Running")]; }));
            if (kind === "eventlisteners")
                return table(["NAME", "AGE", "URL", "AVAILABLE"], res.map(function (r) { var c = r.cols || []; return [r.name, "1h", c[0] || "", c[1] || ""]; }));
            return table(["NAME", "AGE"], res.map(function (r) { return [r.name, (r.cols || [])[0] || "1h"]; }));
        }
        if (!name)
            return "Error: accepts 1 arg(s), received 0";
        if (!res || !res.length)
            return "Error: failed to find " + singular(kind) + ' "' + name + '"';
        var r = res[0];
        if (sub === "describe")
            return "Name:              " + r.name + "\nNamespace:         " + (r.ns || ns) + "\n" + (r.desc || "");
        if (sub === "logs")
            return r.logs || "(no logs: the run never started a pod)";
        if (sub === "start")
            return "PipelineRun started: " + name + "-run-x7q2k\n(and it fails the same way; a fresh run does not change what the pipeline references)";
        if (sub === "delete")
    ...
    function rollouts(sc, pos, f, ns, verb) {
        ...
        if (pos[1] && pos[1] !== "rollouts")
            return 'Error: unknown resource "' + pos[1] + '" for "kubectl argo rollouts ' + verb + '"';
        if (verb === "list")
            return table(["NAME", "STRATEGY", "STATUS", "STEP", "SET-WEIGHT", "READY", "DESIRED", "UP-TO-DATE", "AVAILABLE"], (find(sc, "rollouts", "", ns) || []).map(function (r) { var g = r.canary || {}; return [r.name, "Canary", g.status || "", g.step || "", g.weight || "", "2/2", "2", "2", "2"]; }));
        if (!name)
            return "Error: accepts 1 arg(s), received 0";
        var res = find(sc, "rollouts", name, ns);
        if (!res || !res.length)
            return 'Error: rollouts.argoproj.io "' + name + '" not found';
        var r = res[0], g = r.canary || {};
        if (verb === "get")
            return g.get || ("Name:            " + r.name + "\nNamespace:       " + (r.ns || ns) + "\nStatus:          " + (g.status || "") + "\nStrategy:        Canary");
        if (verb === "retry" || verb === "promote" || verb === "abort" || verb === "undo" || verb === "restart")
    ...
    function trace(sc, pos, f, ns, verb) {
        ...
        var kind = pos[1] || "", name = pos[2];
        if (!kind || !name)
            return "Error: accepts 2 arg(s): crossplane beta trace <kind> <name>";
        var res = find(sc, kind, name, ns);
        if (!res || !res.length)
            return 'crossplane: error: ' + kindName(kind) + '.' + (kind === "appenvironments" ? "platform.lab.local" : "") + ' "' + name + '" not found in namespace ' + ns;
        var r = res[0];
