...
(function () {
    ...
    function closeDialog() { dlg = null; dialog.hidden = true; if (scene === "map")
        stage.focus(); }
    /* ── drawing ────────────────────────────────────────────── */
    function draw() {
    ...
    function renderLog(e) {
        ...
        var out = esc(e.out || "");
        if (window.CNPE_SYNTAX) {
            try {
                out = window.CNPE_SYNTAX.highlight(out, /^(argocd|flux|tkn|crossplane|cosign)/.test(e.cmd) ? "bash" : "bash");
            }
            catch (er) { }
        }
        if (e.tell) {
            ...
            if (at >= 0)
                out = out.slice(0, at) + '<span class="tell">' + t + "</span>" + out.slice(at + t.length);
        }
    ...
    function build() {
        ...
        if (window.CNPE_THEME && !themeWired) {
            themeWired = true;
            window.CNPE_THEME.onChange(function () { readPalette(); dirty = true; draw(); if (scene === "battle" && battle)
                paintBattle(); });
        }
