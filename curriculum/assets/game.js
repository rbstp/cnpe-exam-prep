...
(function () {
    ...
    var audio = null, soundOn = false, soundRun = 0;
    var voices = new Set();
    /** listeners, observers and timers the mount holds, undone in order by unmount() */
    var undo = [];
    var timers = []; // one-shot timers (a floating number's fallback, a result card's delay), with what they run so settle() can fire them
    var mounts = 0;
    ...
    function later(fn, ms) {
        var t = { id: 0, fn: fn };
        t.id = window.setTimeout(function () { var i = timers.indexOf(t); if (i >= 0)
    ...
    function settle() {
        if (!host)
            return;
        var due = timers;
        timers = []; // what they run may set new ones, which stay pending
    ...
    function cue(kind) {
        if (!soundOn || !audio || audio.state !== "running" || document.hidden)
            return;
    ...
    async function toggleSound() {
        ...
        try {
            ...
            var ac = audio;
            await ac.resume();
            if (run !== soundRun || audio !== ac || !host)
                return;
            cue("scene");
        }
        catch (error) {
            if (run !== soundRun || !host)
                return;
            soundOn = false;
            paintSound();
            live.textContent = "Sound could not start. You can keep playing silently.";
            if (soundBtn)
                soundBtn.title = "Sound unavailable. Try enabling sound again.";
            console.warn("CNPE Quest could not enable audio:", error);
        }
    }
    function transition(label, kind) {
        if (!transitionEl)
            return;
        ...
        later(function () { if (run === transitionRun)
            overlay.hidden = true; }, 650);
    }
    /* ── the overworld ──────────────────────────────────────── */
    ...
    function nextGoal() {
        if (trackedTown) {
            ...
            if (tracked && !has("towns", tracked.sec))
                return { x: tracked.x, y: tracked.y, what: "the trial in " + tracked.name };
            if (tracked && !wins(tracked.dungeon))
                return { x: tracked.door.x, y: tracked.door.y, what: "the dungeon of " + tracked.name };
            trackedTown = null;
        }
        var best = null, bd = 1e9;
        var offer = function (x, y, what) {
            ...
            if (d < bd) {
                bd = d;
                best = { x: x, y: y, what: what };
            }
        };
        D.towns.forEach(function (t) {
            if (!has("towns", t.sec))
                offer(t.x, t.y, "the trial in " + t.name);
            else if (!wins(t.dungeon))
                offer(t.door.x, t.door.y, "the dungeon of " + t.name);
        });
        if (best)
            return best;
    ...
    function wayTo(gl, words) {
        var dx = gl.x - player.x, dy = gl.y - player.y;
        if (words) {
            var ns = dy < 0 ? "north" : dy > 0 ? "south" : "", ew = dx > 0 ? "east" : dx < 0 ? "west" : "";
    ...
    // over from the tile before it across STEP_MS, through whole pixels on every
    // animation frame. The walk cycle is keyed to the step's STEP_SUBS
    // sub-positions: one half of a stride through the first two, the pass through
    // the third, the other half through the last two, and the hero lands standing
    // (WALK_CYCLE names ART.hero's frame for each). A step asked for while one is
    // in flight waits for it and starts the moment it lands, so a held key walks
    // tile by tile and never skips one. Under reduced motion the step is instant.
    var STEP_MS = 120, STEP_SUBS = 5, WALK_CYCLE = [1, 1, 3, 2, 2];
    ...
    var route = [], routeRun = 0;
    function cancelRoute() { route = []; routeRun++; }
    function followRoute() {
        if (walk || scene !== "map" || dlg || !route.length)
            return;
    ...
    function walkTo(x, y) {
        cancelRoute();
        if (!walkable(x, y)) {
            live.textContent = "That terrain cannot be crossed. Choose a road or open ground.";
            requestDraw();
            return;
        }
        ...
        var parents = new Int32Array(mapW * mapH);
        parents.fill(-1);
        parents[start] = start;
        ...
        for (var i = 0; i < queue.length && parents[goal] < 0; i++) {
            ...
            offsets.forEach(function (offset) {
                ...
                if (key < 0 || parents[key] >= 0 || !walkable(nx, ny))
                    return;
                // Travel never enters an intermediate town or encounter without a choice.
                if (key !== goal && (townAt(nx, ny) || doorAt(nx, ny) || keepAt(nx, ny) || gateAt(nx, ny)))
                    return;
                parents[key] = current;
                queue.push(key);
            });
        ...
        if (parents[goal] < 0) {
            live.textContent = "There is no open route there. Try a nearer road.";
            requestDraw();
            return;
        }
        for (var at = goal; at !== start; at = parents[at])
            route.push({ x: at % mapW, y: Math.floor(at / mapW) });
        route.reverse();
    ...
    function move(dx, dy, following) {
        ...
        if (reduceMotion) {
            ...
            if (route.length) {
                var run = routeRun;
                later(function () { if (run === routeRun)
                    followRoute(); }, STEP_MS);
            }
            return;
        }
    ...
    /** how far the step in flight has come: 0 at the tile it left, 1 on the tile it ends on */
    function stepProgress(now) {
        if (!walk)
            return 1;
        return Math.min(1, (now - walk.t0) / STEP_MS);
    }
    /** the sub-position that progress falls in, 0 to STEP_SUBS - 1: the walk cycle's key */
    function stepSub(p) { return Math.min(STEP_SUBS - 1, Math.floor(p * STEP_SUBS)); }
    /** the sprite's, and so the camera's, pixel offset from the tile the step ends on: whole pixels, for this frame */
    ...
    var EASE_MS = 180;
    var ease = null; // the ease in flight: from (fx, fy) to the player's camera; t0 is set by its first frame
    var lastCam = null; // the camera as last drawn
    ...
    function startEase() {
        ease = null;
        if (!lastCam || reduceMotion || walk)
            return;
        var want = camera(player.x * TILE, player.y * TILE);
        if (want.x === lastCam.x && want.y === lastCam.y)
            return;
    ...
    function renderTerrain() {
        ...
        if (!k)
            return; // before the sweep is canceled: a render that cannot start must not strand one
        cancelSweep();
    ...
    function restoreMini(m, x, y, w, h) {
    ...
    /** the backing store behind the minimap's CSS box: whole device pixels per tile, like fitCanvas() */
    function fitMini() {
        if (!mini)
            return;
        var dpr = window.devicePixelRatio || 1, cssW = mini.clientWidth || miniBoxW; // hidden off the map, it has no width: the stylesheet's stands in
        var s = Math.max(1, Math.min(4, Math.round(cssW * dpr / mapW)));
        if (s === miniScale && mini.width === mapW * s)
            return;
        miniScale = s;
        mini.width = mapW * s;
        mini.height = mapH * s; // sizing clears it
        if (miniBase) {
            paintMini();
            drawMinimap();
        }
    ...
    function drawMinimap(goal) {
        ...
        if (miniFrame.x >= 0) {
            restoreMini(m, miniFrame.x, miniFrame.y, VW, 1);
            restoreMini(m, miniFrame.x, miniFrame.y + VH - 1, VW, 1);
            restoreMini(m, miniFrame.x, miniFrame.y, 1, VH);
            restoreMini(m, miniFrame.x + VW - 1, miniFrame.y, 1, VH);
        }
        if (miniGoal.x >= 0)
            restoreMini(m, miniGoal.x - 2, miniGoal.y - 2, 5, 5);
        if (miniDot.x >= 0)
            restoreMini(m, miniDot.x - 1, miniDot.y - 1, 3, 3);
        var c = lastCam || camera(player.x * TILE, player.y * TILE);
        ...
        m.fillRect(fx, fy, VW, 1);
        m.fillRect(fx, fy + VH - 1, VW, 1);
        m.fillRect(fx, fy, 1, VH);
        m.fillRect(fx + VW - 1, fy, 1, VH);
        miniFrame.x = fx;
        miniFrame.y = fy;
        ...
        if (gl && (reduceMotion || (animFrame & 1))) {
            ...
            m.fillRect(gl.x - 2, gl.y - 2, 5, 1);
            m.fillRect(gl.x - 2, gl.y + 2, 5, 1);
            m.fillRect(gl.x - 2, gl.y - 1, 1, 3);
            m.fillRect(gl.x + 2, gl.y - 1, 1, 3);
        }
        if (gl) {
            miniGoal.x = gl.x;
    ...
    function draw() {
        ...
        drawnOff = off;
        drawnSub = walk && p < 1 ? stepSub(p) : -1;
        walkFrame = drawnSub < 0 ? 0 : WALK_CYCLE[drawnSub]; // stride, pass, stride over the sub-positions; standing on landing
        var px = player.x * TILE + off.x, py = player.y * TILE + off.y;
        var cam = camera(px, py), cx = cam.x, cy = cam.y;
        if (ease && (walk || reduceMotion))
            ease = null; // a step owns the camera; reduced motion has no ease
        if (ease) {
            if (!ease.t0)
                ease.t0 = t0;
            var q = Math.min(1, (t0 - ease.t0) / EASE_MS), e = 1 - Math.pow(1 - q, 3); // out: quick off the mark, settling onto the player
            cx = Math.round(ease.fx + (cam.x - ease.fx) * e);
            cy = Math.round(ease.fy + (cam.y - ease.fy) * e);
            if (q >= 1)
                ease = null; // e is 1: this frame is the exact camera
        }
        if (lastCam) {
            lastCam.x = cx;
            lastCam.y = cy;
        ...
        var steps = function (n) { return n + (n === 1 ? " step" : " steps"); };
        ...
        if (walk) {
            if (p >= 1)
                ...
                requestDraw();
        } // the step in flight: land it, or paint its next frame
        else if (ease)
            requestDraw(); // the camera still on its way
    }
    ...
    function tickBeat() { if (!host || reduceMotion || scene !== "map")
        return; beat(); }
    ...
    function fitCanvas() {
        ...
        if (!fullOn && window.innerWidth > 1000 && window.innerHeight >= 650) {
            ...
            host.style.removeProperty("--gm-fit-height");
        fitMini();
    ...
    function setScene(next) {
        ...
        if (journalBtn)
            journalBtn.disabled = next !== "map" && next !== "journal";
        ...
        screen.removeAttribute("data-fx");
        if (onMap) {
            screen.innerHTML = "";
            bt = null;
            tn = null;
    ...
    function openJournal() {
        ...
        D.regions.forEach(function (region) {
            ...
            var meter = el("progress");
            meter.max = towns.length;
            meter.value = cleared;
    ...
    function buildTown(t) {
        ...
        var scenery = backdrop("square", domainOfSec(t.sec), 304);
        root.appendChild(scenery);
    ...
    function paintTown(right, sceneName) {
        ...
        Object.keys(v.items).forEach(function (id) {
            ...
            if (selected)
                v.items[id].setAttribute("aria-current", "true");
            else
                v.items[id].removeAttribute("aria-current");
        });
        v.right.innerHTML = "";
    ...
    function metKey(n) { return "met-" + n.name.toLowerCase().replace(/[^a-z0-9]+/g, "-").slice(0, 48).replace(/^-|-$/g, ""); }
    ...
    function paintTrial() {
        ...
        if (tr.revealed) {
            var streak = 0;
            for (var j = tr.marks.length - 1; j >= 0 && tr.marks[j]; j--)
                streak++;
            col.appendChild(el("p", "gm-feedback " + (tr.marks[tr.i] ? "ok" : "bad"), tr.marks[tr.i] ? "Correct! +5 xp" + (streak > 1 ? " · " + streak + " answers in a row" : "") : "Not quite. The highlighted answer is the right one. This card will return in your drill."));
    ...
    function nextTrial() {
        ...
        paintTown(msg);
        if (passed) {
            transition("Trial cleared · the seal is broken", "reward");
            cue("victory");
        }
    }
    /* ── the battle ─────────────────────────────────────────── */
    function startBattle(chain, opts) {
        if (opts.town && !dungeonOpen(opts.town)) {
            sealedDoor(opts.town);
            return;
        }
        if ((opts.boss && !keepOpen(opts.boss)) || (opts.final && !gateOpen())) {
            say("The gate holds", ["Clear the dungeons and keeps guarding this entrance first."]);
            return;
        }
        if (hp < 1)
            hp = maxHp();
    ...
    function buildBattle() {
        ...
        var scenery = backdrop("battle", b.sc.d);
        arena.appendChild(scenery);
        var action = el("div", "gm-action", "YOUR TURN · Gather evidence. Break the guard.");
        action.setAttribute("role", "status");
        arena.appendChild(action);
        var party = el("div", "gm-party");
        var hero = el("canvas");
        hero.width = 16;
        hero.height = 16;
        hero.setAttribute("aria-hidden", "true");
        party.appendChild(hero);
        party.appendChild(el("span", "", "YOU"));
        arena.appendChild(party);
        ...
        guardWrap.addEventListener("animationend", function (e) { if (e.target === guardWrap)
            guardWrap.classList.remove("fx-shake"); });
        ...
        var execute = el("button", "gm-btn gm-execute", "Cast");
        execute.type = "submit";
        execute.setAttribute("aria-label", "Run command");
    ...
    function paintBattle() {
        ...
        if (t.scId !== sc.id) {
            ...
            if (heroCtx) {
                ...
                heroCtx.drawImage(ART.hero("l", 0), 0, 0);
            }
        ...
        var execute = t.root.querySelector(".gm-execute");
        if (execute)
            execute.disabled = !!b.over;
        ...
        if (fx) {
            ...
            if (fx.name === "stagger")
                pulse(t.guard.parentNode, "shake", 450);
            if (fx.name === "hit")
    ...
    function resultCard(won) {
        ...
        if (level() > b.startLevel)
            card.appendChild(el("p", "gm-level-up", "LEVEL UP! " + b.startLevel + " → " + level() + " · " + title(level())));
    ...
    function setFull(on) {
        ...
        if (on)
            scrollBack = window.pageYOffset || 0;
        fullOn = on;
    ...
    function toggleFull(want) {
        ...
        if (scene === "map")
            stage.focus({ preventScroll: true }); // the keys are the stage's again; the page stays where setFull() put it
    }
    /* ── input ──────────────────────────────────────────────── */
    function keys() {
        ...
        listen(document, "keydown", function (e) {
            ...
            if (e.key === "Escape") {
                if (scene === "map") {
                    ...
                    handled = false;
                else if (scene === "town" || scene === "journal")
                    leaveToMap();
                else
                    handled = false;
                ...
                if (!trial.revealed && /^[1-4]$/.test(e.key) && trial.opts[+e.key - 1])
                    answerTrial(+e.key - 1);
                else if (trial.revealed && e.key === "Enter" && target.tagName !== "BUTTON")
                    nextTrial();
                else
    ...
    function build() {
        ...
        if (typeof AudioContext === "undefined") {
            ...
            soundBtn.title = "This browser does not support synthesized audio";
        }
        ...
        var side = el("div", "gm-side");
        stage.appendChild(side);
        whereEl = el("div", "gm-win gm-where");
        side.appendChild(whereEl);
        ...
        screen.addEventListener("animationend", function (e) { if (e.target === screen)
            screen.classList.remove("fx-flash"); });
        transitionEl = el("div", "gm-transition");
        ...
        if (document.fonts && document.fonts.ready)
            document.fonts.ready.then(function () { fitCanvas(); requestDraw(); });
