...
(function () {
  ...
  /** a slot-map counter, bumped for this browser
      @param {*} owner @param {string} key @param {number} n */
  ...
  /** @param {number} n */
  function addXp(n) { if (n > 0) bump(g(), "xp", Math.round(n)); }
  /** @param {number} n */
  function addGold(n) { var gd = obj(g().gold) || (g().gold = {}); if (n > 0) bump(gd, "e", Math.round(n)); }
  /** @param {number} n */
  function spendGold(n) { if (n > gold()) return false; var gd = obj(g().gold) || (g().gold = {}); bump(gd, "s", n); return true; }
  /** @param {string} id @param {number} n */
  function giveItem(id, n) { var it = obj(g().items) || (g().items = {}); var rec = obj(it[id]) || (it[id] = {}); bump(rec, "g", n); }
  /** @param {string} id */
  function useItem(id) { if (held(id) < 1) return false; var rec = obj(g().items[id]); bump(rec, "u", 1); return true; }
  /** @param {"towns"|"learned"|"flags"} b @param {string} k */
  function tick(b, k) { var m = obj(g()[b]) || (g()[b] = {}); if (m[k]) return false; m[k] = 1; return true; }
  /** @param {"towns"|"learned"|"flags"} b @param {string} k */
  function has(b, k) { var m = obj(g()[b]); return !!(m && m[k]); }
  /** @param {string} id */
  function wins(id) { var w = obj(g().wins), r = w && obj(w[id]); return r ? (+r.n || 0) : 0; }
  /** @param {string} id @param {number} turns */
  ...
  /** @param {CnpeDrillQuestion} q @param {boolean} okAns */
  ...
  /** @param {string} label @param {() => void} fn @param {string} [cls] @return {HTMLButtonElement} */
  function btn(label, fn, cls) {
    var b = /** @type {HTMLButtonElement} */ (el("button", cls || "", label));
    b.type = "button";
  ...
  function str2n(s) { var h = 0; for (var i = 0; i < s.length; i++) h = (h * 31 + s.charCodeAt(i)) >>> 0; return h; }
  ...
  /* ── state ──────────────────────────────────────────────── */
  /** @type {HTMLElement} */ var host;
  /** @type {HTMLElement} */ var stage;
  /** @type {HTMLCanvasElement} */ var canvas;
  /** @type {CanvasRenderingContext2D} */ var ctx;
  /** @type {HTMLElement} */ var hud, whereEl, dialog, screen, live;
  var player = { x: 0, y: 0, face: "d" };
  var scene = "map";                                  // map | town | trial | battle | shop
  /** @type {CnpeGameTown|null} */ var town = null;
  /** @type {*} */ var trial = null;
  /** @type {*} */ var battle = null;
  ...
  /** @param {number} n */
  function stars(n) { var s = ""; for (var i = 0; i < 3; i++) s += i < n ? "▲" : "▽"; return s.replace(/▽/g, "·"); }

  /* the dialogue window: one speaker, pages of text, an optional yes/no */
  /** @param {string} who @param {string[]} pages @param {(() => void)|null} [yes] @param {boolean} [ask] */
  ...
  function draw() {
    ...
    ctx.fillStyle = P.sunk; ctx.fillRect(0, 0, W, H);
    for (var y = 0; y < VH; y++) for (var x = 0; x < VW; x++) drawTile(cx + x, cy + y, x * TILE, y * TILE);
  ...
  /** @param {string} s @param {number} tx @param {number} ty @param {string} color */
  function label(s, tx, ty, color) {
    if (tx < -4 || tx >= VW + 4 || ty < 0 || ty >= VH) return;
    var w = s.length * 5 + 6, x = tx * TILE + TILE / 2 - w / 2, y = ty * TILE - 11;
    if (y < 0) y = ty * TILE + TILE + 1;
    ctx.fillStyle = P.ink; ctx.globalAlpha = 0.85; ctx.fillRect(x, y, w, 10); ctx.globalAlpha = 1;
    ctx.fillStyle = color; ctx.fillText(s, x + 3, y + 1);
  }
  /** @param {number} mx map x @param {number} my map y @param {number} px @param {number} py */
  function drawTile(mx, my, px, py) {
    ...
    switch (t) {
      case "grass": case "flower":
        base(P.s2);
        ctx.fillStyle = P.s3;
        if (r > 0.5) ctx.fillRect(px + Math.floor(r * 12), py + Math.floor(r2 * 12), 2, 1);
        if (r2 > 0.7) ctx.fillRect(px + Math.floor(r2 * 13), py + 3 + Math.floor(r * 10), 1, 2);
        if (t === "flower") { ctx.fillStyle = r > 0.5 ? P.warn : P.bad; ctx.fillRect(px + 4 + Math.floor(r * 8), py + 4 + Math.floor(r2 * 8), 2, 2); }
        break;
      case "road":
        base(P.s3);
        ctx.fillStyle = P.rule; if (r > 0.6) ctx.fillRect(px + Math.floor(r2 * 12), py + Math.floor(r * 12), 2, 2);
        break;
      case "sand":
        base(P.warnDim); ctx.globalAlpha = 0.35; ctx.fillStyle = P.ink; ctx.fillRect(px, py, TILE, TILE); ctx.globalAlpha = 1;
        ctx.fillStyle = P.warn; if (r > 0.5) ctx.fillRect(px + Math.floor(r * 13), py + Math.floor(r2 * 13), 1, 1);
        break;
      case "water":
        base(P.accentDim);
      ...
      case "bridge":
        base(P.accentDim);
        ctx.fillStyle = P.warnDim; ctx.fillRect(px, py + 2, TILE, TILE - 4);
        ctx.fillStyle = P.ink; for (var i = 2; i < TILE; i += 4) ctx.fillRect(px + i, py + 2, 1, TILE - 4);
        break;
      case "cliff":
        base(P.rule);
        ctx.fillStyle = P.rule2;
        ctx.beginPath(); ctx.moveTo(px + 2, py + 14); ctx.lineTo(px + 7 + Math.floor(r * 3), py + 3); ctx.lineTo(px + 14, py + 14); ctx.closePath(); ctx.fill();
        ctx.fillStyle = P.paper3; ctx.fillRect(px + 7 + Math.floor(r * 3), py + 3, 2, 2);
        break;
      case "tree":
        base(P.s2);
        ctx.fillStyle = P.warnDim; ctx.fillRect(px + 7, py + 10, 2, 5);
        ctx.fillStyle = P.ok; ctx.globalAlpha = 0.85;
        ctx.beginPath(); ctx.arc(px + 8, py + 7, 5 + Math.floor(r * 2), 0, Math.PI * 2); ctx.fill();
        ctx.globalAlpha = 1; ctx.fillStyle = P.okLit; ctx.fillRect(px + 6, py + 4, 2, 2);
        break;
      case "town":
        base(P.s2);
        ctx.fillStyle = P.paper2; ctx.fillRect(px + 3, py + 7, 10, 7);
        ctx.fillStyle = P.bad; ctx.beginPath(); ctx.moveTo(px + 2, py + 7); ctx.lineTo(px + 8, py + 2); ctx.lineTo(px + 14, py + 7); ctx.closePath(); ctx.fill();
        ctx.fillStyle = P.ink; ctx.fillRect(px + 7, py + 10, 2, 4);
        ctx.fillStyle = P.warn; ctx.fillRect(px + 4, py + 9, 2, 2); ctx.fillRect(px + 10, py + 9, 2, 2);
        break;
      case "door": {
        base(P.rule);
        var open = doorAt(mx, my) && dungeonOpen(doorAt(mx, my));
        ctx.fillStyle = P.rule2; ctx.fillRect(px + 1, py + 1, TILE - 2, TILE - 1);
        ctx.fillStyle = open ? P.viol : P.sunk; ctx.fillRect(px + 5, py + 6, 6, 9);
        ctx.beginPath(); ctx.arc(px + 8, py + 6, 3, Math.PI, 0); ctx.fill();
        if (!open) { ctx.fillStyle = P.paper3; ctx.fillRect(px + 5, py + 9, 6, 1); ctx.fillRect(px + 5, py + 12, 6, 1); }
        break;
      }
      case "keep": {
        base(P.s3);
        ctx.fillStyle = P.paper3; ctx.fillRect(px + 2, py + 5, 12, 10);
        ctx.fillStyle = P.paper2; ctx.fillRect(px + 2, py + 3, 3, 3); ctx.fillRect(px + 7, py + 3, 2, 3); ctx.fillRect(px + 11, py + 3, 3, 3);
        ctx.fillStyle = P.ink; ctx.fillRect(px + 6, py + 9, 4, 6);
        var kp = keepAt(mx, my);
        ctx.fillStyle = kp && has("flags", "boss-" + kp.d) ? P.ok : P.bad; ctx.fillRect(px + 7, py, 1, 4); ctx.fillRect(px + 8, py, 4, 2);
        break;
      }
      case "gate":
        base(P.s3);
        ctx.fillStyle = P.viol; ctx.fillRect(px + 2, py + 2, 3, 13); ctx.fillRect(px + 11, py + 2, 3, 13); ctx.fillRect(px + 2, py + 2, 12, 3);
        ctx.fillStyle = has("flags", "final") ? P.ok : P.accent; ctx.fillRect(px + 6, py + 6, 4, 9);
        break;
      default:
        base(P.sunk);
    }
  ...
  function drawPlayer(px, py) {
    ctx.fillStyle = P.ink; ctx.globalAlpha = 0.35; ctx.fillRect(px + 4, py + 13, 8, 2); ctx.globalAlpha = 1;
    ctx.fillStyle = P.accent; ctx.fillRect(px + 5, py + 7, 6, 6);        // tunic
    ctx.fillStyle = P.paper; ctx.fillRect(px + 5, py + 2, 6, 5);         // face
    ctx.fillStyle = P.warnDim; ctx.fillRect(px + 4, py + 1, 8, 2);       // hair
    ctx.fillStyle = P.ink;
    if (player.face === "l") ctx.fillRect(px + 5, py + 4, 1, 1);
    else if (player.face === "r") ctx.fillRect(px + 10, py + 4, 1, 1);
    else if (player.face === "d") { ctx.fillRect(px + 6, py + 4, 1, 1); ctx.fillRect(px + 9, py + 4, 1, 1); }
    ctx.fillRect(px + 5, py + 13, 2, 2); ctx.fillRect(px + 9, py + 13, 2, 2);   // boots
  }
  /** the monster: a symmetric blob seeded by the scenario id, on the 96px enemy canvas
      @param {HTMLCanvasElement} c @param {CnpeGameScenario} sc */
  function drawMonster(c, sc) {
    var k = c.getContext("2d");
    if (!k) return;
    var seed = str2n(sc.id), n = 12, cell = 8;
    k.setTransform(1, 0, 0, 1, 0, 0); k.clearRect(0, 0, c.width, c.height);
    ...
    for (var y = 0; y < n; y++) for (var x = 0; x < n / 2; x++) {
      ...
      if (v < 0.72 - dy * 0.5 - dx * 0.3) {
        ...
        k.fillRect(x * cell, y * cell, cell, cell); k.fillRect((n - 1 - x) * cell, y * cell, cell, cell);
      }
    }
    k.fillStyle = eye; var ey = 3 + (seed % 3), ex = 3 + (seed % 2);
    k.fillRect(ex * cell, ey * cell, cell, cell); k.fillRect((n - 1 - ex) * cell, ey * cell, cell, cell);
    k.fillStyle = P.ink; k.fillRect(ex * cell + 3, ey * cell + 3, 3, 3); k.fillRect((n - 1 - ex) * cell + 3, ey * cell + 3, 3, 3);
  }
  function paintHud() {
    var mh = maxHp(), cells = "";
  ...
  function setScene(next) {
    ...
    if (onMap) { screen.innerHTML = ""; dirty = true; draw(); paintHud(); savePos(true); }
  ...
  function paintTown(right) {
    ...
    var item = function (label, meta, fn, cls) {
      var li = el("li"); var b = btn(label + (meta ? '<span class="k">' + meta + "</span>" : ""), fn, cls || ""); li.appendChild(b); menu.appendChild(li); return b;
    };
    ...
    var read = el("li"); var a = /** @type {HTMLAnchorElement} */ (el("a", "", "Read the section" + '<span class="k">' + esc(t.sec) + "</span>"));
    ...
    if (right) rightCol.appendChild(right);
    else rightCol.appendChild(el("div", "gm-lines", "<p>" + esc(t.blurb || ("The people here work on " + (nav ? nav.title.toLowerCase() : "the platform") + ".")) + "</p><p class=\"gm-note\">Talk to learn the ideas and the commands. Pass the trial to open the dungeon. Read the section itself when a question stumps you.</p>"));
  ...
  function shopMenu() {
    ...
    Object.keys(D.items).forEach(function (id) {
      ...
      var b = btn('<span class="nm">' + esc(it.name) + '<span class="p">' + it.price + "g</span></span>" +
        ...
        function () {
          if (it.permanent && owned) { paintTown(shopMenu()); return; }
          if (!spendGold(it.price)) { paintTown(wrapNote(shopMenu(), "Not enough gold. Trials and battles pay.", "warn")); return; }
          giveItem(id, 1); save();
  ...
  /** @param {HTMLElement} w @param {string} msg @param {string} cls */
  ...
  function paintTrial() {
    ...
    if (tr.revealed) { var nb = /** @type {HTMLElement} */ (acts.querySelector("button")); if (nb) nb.focus(); }
  ...
  /* ── the battle ─────────────────────────────────────────── */
  /** @param {string[]} chain @param {{town?: CnpeGameTown, boss?: CnpeGameRegion, final?: boolean}} opts */
  ...
  function paintBattle() {
    ...
    var mc = /** @type {HTMLCanvasElement} */ (el("canvas")); mc.width = 96; mc.height = 96; mc.setAttribute("role", "img"); mc.setAttribute("aria-label", "The monster " + sc.name);
    drawMonster(mc, sc);
    ...
    side.style.flex = "1"; side.style.minWidth = "0";
    ...
    var input = /** @type {HTMLInputElement} */ (el("input")); input.type = "text"; input.autocomplete = "off"; input.spellcheck = false;
    input.setAttribute("aria-label", "Command"); input.placeholder = "type a command, or pick one from Inspect / Fix";
    if (b.pending) { input.value = b.pending; b.pending = null; }
    ...
    if (b.mode === "menu" || b.mode === "typed") input.focus();
    else focusFirst(sub);
  }
  ...
  /** @param {string|null} value @param {string|undefined} ns */
  ...
  function build() {
    ...
    canvas = /** @type {HTMLCanvasElement} */ (el("canvas"));
    ...
    ab.appendChild(btn("A<b>act</b>", function () { if (scene === "map") { stage.focus(); act(); } else { var f = /** @type {HTMLElement} */ (screen.querySelector("button:focus, a:focus")) || /** @type {HTMLElement} */ (screen.querySelector(".gm-menu button, .gm-opt, .gm-acts button")); if (f) f.click(); } }));
    pad.appendChild(ab);
  ...
  window.CNPE_GAME = {
    mount: function () {
      ...
      scene = "map"; town = null; trial = null; battle = null; dlg = null;
      build();
      hp = maxHp();
      loadPos();
      keys();
