...
(function () {
    ...
    function pad(g) {
        ...
        for (var y = 0; y < TILE; y++) {
            var row = g[y] || "";
            while (row.length < TILE)
                row += ".";
            out.push(row.slice(0, TILE));
        }
        return out;
    }
        ...
        HERO[face] = HERO[face].map(function (g, frame) {
            ...
            return g.map(function (row, y) {
                return row.split("").map(function (ch, x) {
                    var yy = y + lift;
                    if (ch === "H")
                        return yy === 2 && x > 4 && x < 10 ? "J" : x > 9 || yy === 5 ? "h" : "H";
                    if (ch === "S")
                        return yy === 7 || x > 9 ? "q" : yy === 4 && x < 8 ? "F" : "S";
                    if (ch === "T") {
                        if (yy === 8)
                            return x === 7 || x === 8 ? "G" : "P";
                        if (x < 5 || x > 10)
    ...
    Object.keys(ENEMIES).forEach(function (family) {
        ...
        ENEMIES[family] = g.map(function (row, y) {
            return row.split("").map(function (ch, x) {
                if (ch === "e")
                    return (g[y - 1] || "")[x] !== "e" && x % 3 !== 0 ? "E" : "e";
                if (ch === "a")
                    return (x + y) % 5 === 0 ? "A" : "a";
                if (ch !== "b")
                    return ch;
                var left = row[x - 1], above = (g[y - 1] || "")[x];
                if ((left === "d" || above === "d") && x < 21 && y < 25)
                    return "m";
                if (x > 21 || y > 24)
                    return (x + y) % 4 === 0 ? "b" : "h";
                if (family === "storage")
                    return y % 5 === 0 ? "h" : y % 5 === 1 && x % 7 !== 0 ? "m" : "b";
                if (family === "networking")
                    return y % 3 === 0 && (x + (y % 2)) % 3 === 0 ? "l" : (x + y) % 4 === 0 ? "h" : "b";
                if (family === "gitops")
                    return (x + Math.floor(y / 4)) % 5 < 2 ? "m" : x % 4 === 0 ? "h" : "b";
                if (family === "observability")
                    return (x * 3 + y * 5) % 11 < 3 ? "m" : (x + y) % 5 === 0 ? "h" : "b";
    ...
    var VILLAGER = ["................", "......kkkk......", ".....kKKKkk.....", ".....kppppk.....", ".....kpepek.....", "......pppq......", ".......qq.......", ".....bAyyab.....",
    ...
    [TOWN, HOUSE, INN, SHOP, WELL, BED, COUNTER].forEach(function (g) {
        g.forEach(function (row, y) {
            g[y] = row.split("").map(function (ch, x) {
                if (ch === "b")
                    return row[x + 1] !== "b" ? "r" : y % 2 === 0 && x % 3 !== 0 ? "B" : "b";
                if (ch === "p")
                    return x % 6 === 1 || y === 7 ? "k" : y % 4 === 0 ? "P" : "p";
                if (ch === "y")
                    return x % 2 === 0 ? "Y" : "y";
                return ch;
            }).join("");
    ...
    function sky(k, w, h, c, d) {
        var bands = 9, bandH = Math.ceil(h / bands), last = c.sky;
        for (var i = 0; i < bands; i++) {
            var col = mix(c.sky, c.horizon, i / (bands - 1)), y = i * bandH;
            block(k, col, 0, y, w, bandH);
            if (i)
                for (var x = i % 2; x < w; x += 4) {
                    block(k, last, x, y, 2, 1);
                    block(k, last, x + 2, y + 1, 1, 1);
                }
            last = col;
        }
        var sx = Math.floor(w * 0.77), sy = Math.max(4, Math.floor(h * 0.13));
        block(k, c.sun, sx + 3, sy, 8, 2);
        block(k, c.sun, sx + 1, sy + 2, 12, 2);
        block(k, c.sun, sx, sy + 4, 14, 6);
        block(k, c.sun, sx + 2, sy + 10, 10, 2);
        for (var n = -1; n < w / 68 + 1; n++) {
            ...
            block(k, c.cloud, cx + 4, cy + 3, 30, 4);
            block(k, c.cloud, cx + 12, cy, 14, 4);
            block(k, c.cloud, cx - 5, cy + 7, 20, 1);
            block(k, c.cloudShade, cx + 26, cy + 9, 25, 1);
        }
    }
    function mountains(k, w, baseline, height, c, d, far) {
        ...
        for (var x = 0; x < w; x += 2) {
            ...
            if (!far && phase < 0.42)
                block(k, c.ridge, x, top, 2, Math.max(1, rise * slope * 0.32));
            if (!far && d === 3 && slope > 0.72)
                block(k, c.cloud, x, top, 2, 1 + (slope - 0.72) * 18);
        }
    }
    function scenicTree(k, x, y, size, c, pine, distant) {
        ...
        if (!distant)
            block(k, c.barkLit, x - 1, y - size * 0.43, 1, size * 0.43);
        if (pine) {
            for (var tier = 0; tier < 3; tier++) {
                ...
                for (var r = 0; r < size * 0.4; r += 2) {
                    ...
                    if (r % 4 === 0)
                        block(k, light, x - width + 1, top + r, Math.max(1, width * 0.65), 1);
                }
            }
        }
        else {
            ...
            for (var j = 0; j < 8; j++) {
                ...
                if (!distant && j < 3)
                    block(k, light, xx + 1, yy - 1, 2, 1);
            }
        }
    }
    function outdoor(k, w, h, d, c) {
        ...
        for (var x = -5; x < w + 15; x += 11)
            scenicTree(k, x, horizon + 3, 11 + noise(x + d) * 13, c, true, true);
        ...
        if (d === 2 || d === 4) {
            ...
            for (x = 0; x < w; x += 11) {
                ...
                if (d === 4) {
                    block(k, c.grass, x + 3, waterY + 3, 6, 2);
                    block(k, c.leafDark, x + 4, waterY - 2, 1, 4);
                }
            }
        }
        return horizon;
    }
    function windowPane(k, x, y, w, h, s) {
        block(k, s.k, x - 1, y - 1, w + 2, h + 2);
        block(k, s.y, x, y, w, h);
        block(k, s.Y, x, y, 1, h - 1);
        block(k, s.k, x + Math.floor(w / 2), y, 1, h);
        block(k, s.k, x, y + Math.floor(h / 2), w, 1);
    ...
    function building(k, x, top, w, h, kind, s) {
        ...
        block(k, s.p, x + 3, wall, w - 7, h - roof - 2);
        block(k, s.P, x + w - 10, wall, 6, h - roof - 2);
        for (var y = wall + 2; y < base - 2; y += 5) {
            block(k, s.P, x + 3, y, w - 7, 1);
            for (var brick = x + 4; brick < x + w - 5; brick += 9)
                block(k, s.C, brick + (y % 2) * 3, y + 2, 3, 1);
        }
        for (var post = x + 3; post < x + w - 4; post += Math.max(7, Math.floor((w - 7) / 4))) {
            block(k, s.k, post, wall, 2, h - roof - 1);
            block(k, s.r, post, wall + 2, 1, h - roof - 5);
        }
        block(k, s.k, x + 3, base - 6, w - 7, 2);
        block(k, s.K, x + 1, base - 2, w + 1, 4);
        block(k, s.C, x + 2, base - 2, w - 1, 1);
        for (var stone = x + 4; stone < x + w; stone += 6)
            block(k, s.c, stone, base, 4, 1);
        ...
        for (var r = 0; r < roof; r++) {
            ...
            if (r % 4 === 0)
                block(k, tile, x + w - 3, top + r, 2, 1);
        }
        block(k, s.k, x - 1, wall, w + 3, 3);
        block(k, tileLit, x - 1, wall, w - 2, 1);
        block(k, tileShade, x + 4, top - 1, w - 7, 2);
        block(k, tileLit, x + 5, top - 1, w - 9, 1);
        for (var peg = x + 3; peg < x + w - 3; peg += 5)
            block(k, s.p, peg, wall + 2, 1, 1);
        var chimney = x + w - 13;
        block(k, s.K, chimney, top - 6, 6, 13);
        block(k, s.c, chimney, top - 6, 4, 12);
        block(k, s.C, chimney - 1, top - 6, 8, 2);
        block(k, s.k, chimney + 1, top - 6, 4, 1);
        for (y = top - 3; y < top + 5; y += 3)
            block(k, s.K, chimney + 2, y, 3, 1);
        block(k, alpha(s.w, 0.36), chimney + 1, top - 10, 4, 2);
        block(k, alpha(s.w, 0.22), chimney + 3, top - 14, 5, 2);
        var dormer = x + w * 0.42 - 5, dormerY = top + Math.max(4, roof * 0.32);
        block(k, tileShade, dormer - 2, dormerY + 5, 16, 10);
        block(k, s.p, dormer, dormerY + 3, 11, 9);
        for (r = 0; r < 6; r++)
            block(k, r % 2 ? tileShade : tileLit, dormer + 5 - r, dormerY + r, 1 + r * 2, 1);
        windowPane(k, dormer + 3, dormerY + 7, 5, 4, s);
        windowPane(k, x + 7, wall + 6, 6, 7, s);
        windowPane(k, x + w - 15, wall + 6, 6, 7, s);
        block(k, s.a, x + 5, wall + 6, 2, 7);
        block(k, s.a, x + w - 9, wall + 6, 2, 7);
        block(k, s.k, door - 1, base - 14, 10, 14);
        block(k, s.r, door, base - 12, 8, 12);
        block(k, s.B, door + 1, base - 11, 1, 10);
        block(k, s.k, door + 4, base - 12, 1, 12);
        block(k, s.y, door + 6, base - 6, 1, 2);
        block(k, s.C, door - 2, base + 2, 13, 2);
        block(k, s.K, door - 3, base + 4, 15, 1);
        if (kind === 1) {
            block(k, s.k, x + w - 1, wall + 5, 9, 1);
            block(k, s.k, x + w + 5, wall + 5, 1, 4);
            block(k, s.y, x + w + 1, wall + 8, 9, 8);
            block(k, tileShade, x + w + 2, wall + 9, 7, 6);
            block(k, s.Y, x + w + 3, wall + 11, 5, 1);
            block(k, s.Y, x + w + 3, wall + 10, 1, 4);
            block(k, s.Y, x + w + 6, wall + 12, 2, 2);
        }
        else if (kind === 2) {
            ...
            block(k, s.k, x + 3, wall + 10, 1, h - roof - 8);
            block(k, s.k, x + w - 3, wall + 10, 1, h - roof - 8);
        }
    }
    function paving(k, w, top, h, c) {
        block(k, c.roadShade, 0, top, w, h);
        for (var y = top + 1, row = 0; y < top + h; y += 4, row++) {
            for (var x = -(row % 2) * 7; x < w; x += 14) {
                block(k, c.road, x, y, 13, 3);
                block(k, c.roadLit, x + 1, y, 10, 1);
            }
        }
    }
    function ruin(k, x, y, height, c, arch) {
        var pillars = arch ? 2 : 1;
        for (var i = 0; i < pillars; i++) {
            var xx = x + i * 22, top = y - height + (i ? 3 : 0);
            block(k, c.ink, xx - 1, top, 9, y - top);
            block(k, c.stone, xx, top, 6, y - top);
            block(k, c.stoneLit, xx, top, 2, y - top);
            for (var r = top + 4; r < y; r += 5)
                block(k, c.stoneShade, xx + 2, r, 5, 1);
            block(k, c.stone, xx - 2, y - 2, 11, 3);
            block(k, c.leaf, xx + 4, y - 9, 3, 9);
            block(k, c.leafLit, xx + 6, y - 6, 2, 2);
        }
        if (arch) {
            ...
            block(k, c.stoneLit, x - 1, y - height, 4, 2);
            block(k, c.stone, x + 3, y - height + 1, 5, 3);
        }
    }
    function pavedArea(k, x, y, w, h, c) {
        block(k, c.roadShade, x - 1, y - 1, w + 2, h + 2);
        k.save();
        k.beginPath();
        k.rect(Math.floor(x), Math.floor(y), Math.ceil(w), Math.ceil(h));
        k.clip();
        k.translate(Math.floor(x), Math.floor(y));
        paving(k, w, 0, h, c);
        k.restore();
    }
    function garden(k, x, y, w, h, c, s) {
        block(k, c.leafDark, x, y, w, h);
        block(k, c.bark, x + 1, y + 1, w - 2, h - 2);
        for (var row = y + 3; row < y + h - 2; row += 5) {
            block(k, c.roadShade, x + 2, row + 2, w - 4, 1);
            for (var at = x + 3; at < x + w - 3; at += 5) {
                block(k, c.leaf, at - 1, row, 4, 3);
                block(k, c.leafLit, at, row - 1, 2, 3);
                block(k, (at + row) % 3 === 0 ? s.y : s.B, at, row - 1, 2, 1);
            }
        }
        block(k, c.barkLit, x, y + h - 2, w, 1);
        for (at = x; at <= x + w; at += 7) {
            block(k, c.bark, at, y + h - 5, 2, 7);
            block(k, c.barkLit, at, y + h - 5, 1, 5);
        }
    }
    function barrel(k, x, y, s) {
        block(k, s.x, x + 1, y + 8, 9, 3);
        block(k, s.k, x, y + 1, 8, 8);
        block(k, s.r, x + 1, y, 6, 10);
        block(k, s.B, x + 2, y + 1, 2, 7);
        block(k, s.p, x + 1, y, 6, 2);
        block(k, s.k, x + 2, y + 1, 4, 1);
        block(k, s.K, x, y + 3, 8, 1);
        block(k, s.c, x, y + 7, 8, 1);
    }
    function fountain(k, x, y, c, s) {
        block(k, s.x, x - 11, y + 5, 25, 8);
        block(k, s.K, x - 12, y - 2, 24, 11);
        block(k, s.c, x - 10, y - 5, 20, 16);
        block(k, s.C, x - 9, y - 5, 18, 2);
        block(k, s.C, x - 12, y - 2, 2, 7);
        block(k, c.water, x - 9, y - 2, 18, 9);
        block(k, c.water, x - 6, y - 3, 12, 11);
        block(k, c.foam, x - 8, y, 5, 1);
        block(k, c.foam, x + 2, y + 5, 5, 1);
        block(k, s.C, x - 9, y + 8, 18, 1);
        block(k, s.K, x - 10, y + 10, 20, 1);
        block(k, s.K, x - 3, y - 6, 6, 12);
        block(k, s.C, x - 2, y - 6, 4, 10);
        block(k, s.y, x - 4, y - 9, 8, 3);
        block(k, s.Y, x - 2, y - 12, 4, 4);
        block(k, c.foam, x - 5, y - 4, 1, 6);
    ...
    function townScene(k, w, h, d, talk) {
        ...
        block(k, c.stoneShade, canal - 3, 0, 22, h);
        block(k, c.water, canal, 0, 15, h);
        for (y = 0; y < h; y += 4) {
            block(k, c.stoneLit, canal - 3, y, 2, 3);
            block(k, c.stone, canal + 16, y, 3, 3);
            block(k, c.foam, canal + 3 + y % 3, y + 1, 5, 1);
            block(k, c.sky, canal + 9, y + 3, 4, 1);
        }
        ...
        for (y = street; y < street + (full ? 20 : 12); y += 3) {
            ...
            block(k, c.roadShade, canal + 4, y + 1, 1, 1);
        }
        block(k, c.leafDark, canal - 7, street - 3, 31, 2);
        block(k, c.barkLit, canal - 7, street - 4, 31, 1);
        block(k, c.leafDark, canal - 7, street + (full ? 20 : 12), 31, 2);
        for (x = canal - 7; x <= canal + 24; x += 10) {
            block(k, c.barkLit, x, street - 5, 2, 5);
            block(k, c.barkLit, x, street + (full ? 17 : 9), 2, 6);
        }
        ...
        for (x = -7; x < w + 12; x += 14) {
            ...
            if (full)
                scenicTree(k, x + 4, h + 8, 27 + noise(x + 9) * 6, c, d === 3, false);
        }
        for (var n = 0; n < 3; n++) {
    ...
    function battleScene(k, w, h, d) {
        ...
        for (x = 0; x < w; x += 9) {
            block(k, c.grassShade, x, floor - 2, 6, 2);
            block(k, c.grassLit, x + 1, floor - 3, 2, 1);
        }
        for (x = -5; x < w; x += 28) {
            block(k, c.leafDark, x, h - 3, 17, 3);
            block(k, c.leaf, x + 3, h - 5, 9, 3);
            block(k, c.leafLit, x + 5, h - 6, 2, 3);
            if (d === 4) {
                block(k, c.barkLit, x + 10, h - 10, 1, 7);
                block(k, c.roadShade, x + 10, h - 11, 2, 3);
            }
            if (d === 3 || d === 5) {
                block(k, c.stoneShade, x + 17, h - 4, 7, 4);
                block(k, c.stoneLit, x + 18, h - 5, 4, 1);
            }
        }
    }
    function interior(k, w, h, d, inn) {
        ...
        for (var y = 1, row = 0; y < floor; y += 6, row++) {
            for (var x = -(row % 2) * 9; x < w; x += 18) {
                block(k, wall, x, y, 17, 5);
                block(k, s.P, x + 2, y, 12, 1);
            }
        }
        block(k, s.k, 0, floor, w, h - floor);
        for (y = floor + 1, row = 0; y < h; y += 5, row++) {
            for (x = -(row % 2) * 17; x < w; x += 34) {
                block(k, wood, x, y, 33, 4);
                block(k, woodLit, x + 1, y, 30, 1);
                block(k, s.k, x + 3, y + 2, 1, 1);
                block(k, woodLit, x + 14, y + 2, 8, 1);
            }
        }
        block(k, s.k, 0, floor - 4, w, 4);
        block(k, woodLit, 0, floor - 4, w, 1);
        block(k, s.k, 0, 2, w, 4);
        block(k, woodLit, 0, 2, w, 1);
        for (x = 7; x < w; x += 60) {
            block(k, s.k, x, 0, 4, floor);
            block(k, woodLit, x, 6, 1, floor - 10);
            var wy = Math.max(9, floor - 32);
            block(k, s.k, x + 17, wy - 2, 22, 24);
            block(k, c.sky, x + 19, wy, 18, 20);
            block(k, c.horizon, x + 19, wy + 10, 18, 10);
            block(k, c.forest, x + 19, wy + 17, 18, 3);
            block(k, c.cloud, x + 22, wy + 4, 8, 2);
            block(k, c.cloud, x + 26, wy + 2, 4, 2);
            block(k, s.k, x + 27, wy, 2, 20);
            block(k, s.k, x + 19, wy + 9, 18, 2);
            block(k, s.C, x + 16, wy + 20, 24, 2);
            if (inn) {
                block(k, s.b, x + 14, wy - 2, 5, 21);
                block(k, s.B, x + 15, wy, 1, 17);
                block(k, s.b, x + 37, wy - 2, 5, 21);
                block(k, s.B, x + 38, wy, 1, 17);
                block(k, s.y, x + 14, wy + 13, 5, 1);
                block(k, s.y, x + 37, wy + 13, 5, 1);
            }
        }
        var rugX = Math.floor(w * 0.36), rugW = Math.floor(w * 0.3), rugY = full ? floor + 61 : floor + 5;
        var rugH = Math.max(7, h - rugY - 4);
        block(k, s.k, rugX - 1, rugY - 1, rugW + 2, rugH + 2);
        block(k, s.y, rugX, rugY, rugW, rugH);
        block(k, s.b, rugX + 2, rugY + 2, rugW - 4, rugH - 4);
        for (x = rugX + 4; x < rugX + rugW - 4; x += 8) {
            block(k, s.B, x, rugY + 3, 3, rugH - 6);
            block(k, s.y, x + 1, rugY + Math.floor(rugH / 2), 1, 1);
        }
        if (inn) {
            for (x = 14; x < w - 30; x += 68)
                stamp(k, BED, s, x, floor - 8, 2);
            var fx = Math.floor(w * 0.78), fy = floor - 20;
            block(k, s.k, fx - 3, fy - 7, 30, 29);
            block(k, s.c, fx, fy - 7, 24, 27);
            for (y = fy - 5; y < floor; y += 5)
                block(k, s.K, fx, y, 24, 1);
            block(k, s.k, fx + 5, fy + 4, 14, 16);
            block(k, s.b, fx + 6, fy + 12, 12, 7);
            block(k, s.y, fx + 9, fy + 10, 7, 9);
            block(k, s.Y, fx + 11, fy + 8, 2, 10);
            block(k, s.k, fx - 2, fy, 28, 3);
            block(k, s.C, fx - 3, fy - 1, 30, 1);
            stamp(k, VILLAGER, folk(s.a), Math.floor(w * 0.55), floor - 6);
        }
        else {
            for (x = 14; x < w - 15; x += 47) {
                block(k, s.k, x - 1, floor - 22, 36, 24);
                block(k, wood, x, floor - 21, 34, 22);
                for (var shelf = 0; shelf < 2; shelf++) {
                    var shelfY = floor - 18 + shelf * 11;
                    block(k, s.k, x, shelfY + 7, 34, 2);
                    block(k, woodLit, x, shelfY + 7, 34, 1);
                    for (var b = 0; b < 5; b++) {
                        var bottle = b % 3 === 0 ? s.a : b % 3 === 1 ? s.v : s.y;
                        block(k, bottle, x + 3 + b * 6, shelfY + 2, 4, 5);
                        block(k, s.C, x + 4 + b * 6, shelfY, 2, 2);
                        block(k, s.w, x + 3 + b * 6, shelfY + 4, 1, 2);
                    }
                }
            }
            stamp(k, VILLAGER, folk(s.v), Math.floor(w * 0.48), floor - 12);
            for (x = Math.floor(w * 0.27); x < w * 0.69; x += 29)
                stamp(k, COUNTER, s, x, floor - 5, 2);
        }
        for (x = 9; x < w; x += 60) {
        ...
        if (full) {
            block(k, s.k, 0, floor, 5, h - floor);
            block(k, woodLit, 1, floor, 1, h - floor);
            block(k, s.k, w - 5, floor, 5, h - floor);
            block(k, woodLit, w - 5, floor, 1, h - floor);
            if (inn) {
                for (x = 10; x < w - 25; x += 68) {
                    block(k, s.x, x + 2, floor + 8, 4, 29);
                    block(k, s.k, x, floor + 4, 3, 31);
                    block(k, woodLit, x, floor + 4, 1, 30);
                    block(k, s.k, x, floor + 34, 42, 3);
                    block(k, woodLit, x, floor + 34, 42, 1);
                    block(k, s.b, x + 9, floor + 29, 22, 4);
                    block(k, s.y, x + 10, floor + 30, 20, 1);
                }
                for (x = 18; x < w - 25; x += 68) {
                    var tableY = floor + 58;
                    block(k, s.x, x + 2, tableY + 5, 32, 17);
                    block(k, s.k, x, tableY, 31, 18);
                    block(k, wood, x + 1, tableY + 1, 29, 12);
                    block(k, woodLit, x + 2, tableY + 1, 27, 2);
                    block(k, s.k, x + 2, tableY + 15, 3, 7);
                    block(k, s.k, x + 26, tableY + 15, 3, 7);
                    block(k, s.w, x + 5, tableY + 4, 7, 5);
                    block(k, s.P, x + 6, tableY + 5, 5, 2);
                    block(k, s.C, x + 20, tableY + 4, 5, 5);
                    block(k, s.k, x + 21, tableY + 5, 3, 2);
                    block(k, s.y, x + 15, tableY + 2, 2, 6);
                    block(k, s.Y, x + 15, tableY, 2, 2);
                ...
                for (var rowN = 0; rowN < 2; rowN++)
                    for (var shelfN = 0; shelfN < 3; shelfN++) {
                        var bx = 15 + shelfN * Math.max(44, (w - 40) / 3), by = floor + 52 + rowN * 37;
                        block(k, s.x, bx + 3, by + 4, 42, 21);
                        block(k, s.k, bx, by, 41, 22);
                        block(k, wood, bx + 1, by + 1, 39, 16);
                        block(k, woodLit, bx + 1, by + 1, 39, 2);
                ...
                for (x = w - 36; x < w - 10; x += 9)
                    barrel(k, x, floor + 30, s);
            }
    ...
    var api = {
        ...
        flower: function (v, d, frame) {
            ...
            return cached("f" + v + "." + d + "." + frame, function (k) {
                ...
                fill(k, s.g);
                stamp(k, GRASS[(v + 1) % GRASS.length], s, 0, 0);
        ...
        bridge: function (vertical) {
            return cached("b" + (vertical ? "v" : "h"), function (k) {
                ...
                var g = ground(1), s = { t: g.t, P: g.T, k: outline() };
        ...
        ambient: function (kind, frame) {
            ...
            return cached("a" + kind + "." + frame, function (k) {
                var p = P;
                if (kind === "puff")
                    stamp(k, PUFF[frame], { m: alpha(p.paper3, 0.6), M: alpha(p.paper2, 0.4) }, 0, 0);
                else
        ...
        backdrop: function (scene, d, w, h) {
            ...
            return cached("bd" + scene + "." + d + "." + w + "x" + h, function (k) {
                ...
                if (scene === "inn" || scene === "shop")
                    interior(k, sw, sh, d, scene === "inn");
                else if (scene === "battle")
                    battleScene(k, sw, sh, d);
                else
                    townScene(k, sw, sh, d, scene === "talk");
                k.restore();
            }, w, h);
        },
        ...
        check: function () {
            ...
            PUFF.forEach(function (g, i) { sq("puff" + i, g, TILE); });
            TORCH.forEach(function (g, i) { sq("torch" + i, g, TILE); });
            if (FLOWER_SWAY.length !== FLOWER.length || FLOWER_SWAY.some(function (v) { return v.length !== FRAMES - 1; }) || PUFF.length !== FRAMES || TORCH.length !== FRAMES)
                bad.push("ambient-frames");
            ...
            Object.keys(HERO).forEach(function (f) {
                ...
                for (var a = 0; a < fr.length; a++)
                    for (var b = a + 1; b < fr.length; b++)
                        if (fr[a].join() === fr[b].join())
                            same = true;
                if (fr.length !== HERO_FRAMES || same)
            ...
            if (east[0][TILE - 1] === "." || east[TILE - 1][TILE - 1] === "." || east[0][0] !== ".")
                bad.push("shore-east");
            if (south[TILE - 1][0] === "." || south[TILE - 1][TILE - 1] === "." || south[0][0] !== ".")
                bad.push("shore-south");
            return bad;
        }
    };
    window.CNPE_ART = api;
