...
(function (root) {
    ...
    function slots(v) {
        ...
        if (v && typeof v === "object" && !Array.isArray(v)) {
            Object.keys(v).forEach(function (k) {
                var x = +v[k];
                if (x > 0 && ownKey(k) && (k === PRE || SLOT_RE.test(k)))
                    out[k] = x;
            });
        }
        return out;
    ...
    function takeSlots(curV, incV) {
        ...
        Object.keys(theirs).forEach(function (id) {
            if (theirs[id] <= (mine[id] || 0))
                return;
            if (!own(mine, id) && Object.keys(mine).length >= MAXSLOTS)
                return;
            mine[id] = theirs[id];
            moved = true;
        });
    ...
    function streak(store, today) {
        ...
        Object.keys(days).forEach(function (k) {
            if (DAY_RE.test(k) && dayActs(days[k]) > 0)
                q[k] = 1;
        });
        var cur = today && DAY_RE.test(today) ? today : dayKey(new Date());
        if (!q[cur])
            cur = shiftKey(cur, -1); // yesterday still counts as alive
        ...
        if (run && carry && carry.d === cur)
            run += Math.max(0, Math.floor(+carry.n) || 0);
    ...
    function carryRun(s, d, n, edge) {
        ...
        if (cur && DAY_RE.test(cur.d) && (cur.d > d || (cur.d === d && (+cur.n || 0) >= run)))
            return;
        s.run = { d: d, n: run };
    }
    function seedDays(s) {
        ...
        var end = dm.earned;
        if (n < 1 || typeof end !== "string" || !DAY_RE.test(end))
            return;
        var t = dayKey(new Date());
        if (end !== t && end !== shiftKey(t, -1))
            return;
        ...
        for (; i < n; i++) {
            ...
            if (k < edge)
                break; // days live inside the window, and only there
            var d = s.days[k];
            if (!d || typeof d !== "object" || Array.isArray(d))
                d = s.days[k] = {};
            if (!dayActs(d))
                d.c = 10; // the ten cards that earned that day
        }
        if (i < n)
            carryRun(s, shiftKey(end, -i), n - i, edge); // the rest of it ran below the window
    }
    ...
    function restOf(rec) {
        if (!rec || !rec.t || !rec.ok)
            return 0; // never answered, or missed
        ...
        for (var i = STEP.length; i < reps && days < CAP; i++)
            days *= ease;
    ...
    function mergeGame(store, src) {
        ...
        if (ii) {
            ...
            Object.keys(ii).forEach(function (id) {
                ...
                if (!gkey(id) || !rec)
                    return;
                var cur = obj(ci[id]);
                if (!cur && Object.keys(ci).length >= MAXGKEYS)
                    return;
        ...
        ["towns", "learned", "flags"].forEach(function (b) {
            ...
            Object.keys(from).forEach(function (k) {
                if (!gkey(k) || !from[k] || into[k])
                    return;
                if (Object.keys(into).length >= MAXGKEYS)
                    return;
                into[k] = 1;
                g[b] = into;
                n++;
    ...
    function shared(seen, p) {
        ...
        BUCKETS.forEach(function (b) {
            ...
            (seen && seen[b] || []).forEach(function (k) { if (m && own(m, k))
                keep[k] = 1; });
            out[b] = keep;
        });
        return out;
    ...
    function migrate(p) {
        ...
        var ex = obj(p.ex);
        if (ex)
            Object.keys(MOVED_EX).forEach(function (from) {
                if (!own(ex, from))
                    return;
        ...
        if (drill)
            Object.keys(MOVED_DRILL).forEach(function (from) {
                if (!own(drill, from))
                    return;
    ...
    function pickBase(b, progress, rev, uid) {
        if (!b || typeof b !== "object" || Array.isArray(b))
            return null;
        if (b.uid && uid && String(b.uid) !== uid)
            return null;
        var was = +b.rev || 0;
        if (rev < was)
            return null; // the row was deleted and remade
    ...
    function hasAnything(p) {
        if (!p)
            return false;
        if (typeof p.last === "string" && p.last)
            return true;
    ...
    function mergeProgress(store, src, base) {
        ...
        function union(into, from, was, bucket) {
            ...
            if (was)
                Object.keys(was).forEach(function (k) { keys[k] = 1; }); // gone from the remote is a removal
        ...
        ["exam", "exam2"].forEach(function (k) {
            if (!src[k] || typeof src[k] !== "object")
                return;
            if (!store[k] || typeof store[k] !== "object")
                store[k] = {};
            if (!store[k].tasks || typeof store[k].tasks !== "object")
                store[k].tasks = {};
        ...
        if (src.drill && typeof src.drill === "object" && !Array.isArray(src.drill)) {
            if (!store.drill || typeof store.drill !== "object")
                store.drill = {};
            Object.keys(src.drill).forEach(function (k) {
                var inc = src.drill[k];
                if (!ownKey(k) || !inc || typeof inc !== "object" || Array.isArray(inc))
                    return;
        ...
        if (src.drillmeta && typeof src.drillmeta === "object" && !Array.isArray(src.drillmeta)) {
            if (!store.drillmeta || typeof store.drillmeta !== "object")
                store.drillmeta = {};
        ...
        if (src.days && typeof src.days === "object" && !Array.isArray(src.days)) {
            if (!store.days || typeof store.days !== "object" || Array.isArray(store.days))
                store.days = {};
