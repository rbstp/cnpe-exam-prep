...
(function () {
  ...
  var D = window.CNPE_GAME_DATA as CnpeGameData, SIM = window.CNPE_SIM as CnpeSimApi;
  ...
  /** the theme's colours, read live from the stylesheet's custom properties */
  interface Palette {
  ...
  function drawTile(mx: number, my: number, px: number, py: number) {
    var t = tileAt(mx, my), r = hash(mx, my), r2 = hash(my, mx);
    var base = function (c: string) { ctx.fillStyle = c; ctx.fillRect(px, py, TILE, TILE); };
    switch (t) {
  ...
  function drawMonster(c: HTMLCanvasElement, sc: CnpeGameScenario) {
  ...
  function paintTown(right: HTMLElement | null) {
    ...
    var item = function (label: string, meta: string, fn: () => void, cls?: string) {
  ...
  function paintBattle() {
    ...
    var mc = el("canvas") as HTMLCanvasElement; mc.width = 96; mc.height = 96; mc.setAttribute("role", "img"); mc.setAttribute("aria-label", "The monster " + sc.name);
    drawMonster(mc, sc);
