/* CNPE Quest: the content. Compiled into assets/game-data.js by tools/build-ts.sh. The overworld, the towns and their people, the
   ...
   The evidence matchers are regexes over the normalized command (see
...
window.CNPE_GAME_DATA = (function (): CnpeGameData {
  ...
  function tenancy(enforce: string): CnpeGameResource[] {
    return [
  ...
  function dnsPolicy(broken: boolean): CnpeGameResource {
  ...
  /** what the workload builder needs to know about the fault */
  interface WorkloadSpec {
    ready: string; upd: string; avail: string; rsReady: string; podCols: string[]; podDesc: string;
    tpl?: string; dep?: string; logs?: string; prevLogs?: string; logsErr?: string; notRunning?: boolean;
    exec?: Record<string, string>; conds?: string; rsName?: string; podName?: string; extraPods?: CnpeGameResource[];
  }
  /** the drill's deployment, its ReplicaSet and its pod, in whatever state the fault leaves them */
  function workload(o: WorkloadSpec): CnpeGameResource[] {
    var rs = o.rsName || "broken-6b7f9c4d8", pn = o.podName || POD;
    var tpl = o.tpl || ("  containers:\n  - image: " + IMG + "\n    name: nginx-unprivileged\n    ports:\n    - containerPort: 8080\n    resources:\n      requests:\n        cpu: 25m\n        memory: 32Mi\n    securityContext:\n      allowPrivilegeEscalation: false\n      capabilities:\n        drop:\n        - ALL\n  securityContext:\n    runAsNonRoot: true\n    seccompProfile:\n      type: RuntimeDefault");
    var out: CnpeGameResource[] = [
  ...
  SCENARIOS.push({
    ...
    canI: (function () { var m: Record<string, Record<string, string>> = {}; m[SA] = {}; m["system:serviceaccount:team-a:default"] = {}; m["dev-a"] = { pods: "get,list,watch,create,update,patch,delete", deployments: "get,list,watch,create,update,patch,delete", secrets: "get,list" }; return m; })(),
    events: [
  ...
  SCENARIOS.push({
    ...
    wrong: [
      ...
      { match: ["^kubectl create clusterrolebindings \\S+ --clusterrole=cluster-admin"], out: "clusterrolebinding.rbac.authorization.k8s.io created\n(the listener comes up, as cluster-admin; a webhook receiver with root on the cluster is the wrong kind of fix. Bind the ClusterRole Tekton ships, in the namespace)" },
  ...
  function composed(xr: string, kind: string, name: string, ok: boolean): CnpeGameResource {
  ...
  SCENARIOS.push({
    ...
    canI: (function () { var m: Record<string, Record<string, string>> = {}; m[XP_SUBJ] = { "objects.kubernetes.m.crossplane.io": "*", "providerconfigs.kubernetes.m.crossplane.io": "*" }; return m; })(),
    events: [
  ...
  function webPods(labels: string): CnpeGameResource[] {
    return [WEB_POD, WEB_POD2].map(function (n): CnpeGameResource {
  ...
  SCENARIOS.push({
    ...
    answer: "Service web selects app=web, but the Deployment's pods are labeled app=web-frontend, so the EndpointSlice is empty and connections go nowhere. Fix: make the Service's selector match the pod labels (or the labels match the selector).",
    resources: tenancy("baseline").concat([dnsPolicy(false),
  ...
  SCENARIOS.push({
    ...
    answer: "HPA web targets 60% CPU, but the web Deployment's container declares no CPU request, so a utilization percentage cannot be computed: the HPA reports FailedGetResourceMetric / missing request for cpu. Fix: set a CPU request on the container (kubectl set resources).",
    resources: tenancy("baseline").concat([dnsPolicy(false),
    ...
    wrong: [
      { match: ["^kubectl set resources deployments web -n team-a --limits=\\S*cpu=\\S+$", "^kubectl set resources deployments web -n team-a --limits=\\S+$"], out: "deployment.apps/web resource requirements updated\n(a limit is not a request; utilization is measured against the request, and there still is none. Though: with a limit and no request, the request defaults to the limit. Check the pod: if it did, you are done, and if the LimitRange's default took over, it did not)" },
  ...
  SCENARIOS.push({
    ...
    answer: "ExternalSecret db-creds references SecretStore vault, whose service account (eso-team-a) lost the RoleBinding that let it read the backing Secret in the platform-secrets namespace; the store reports SecretSyncedError with a 403 and the ExternalSecret never materializes the Secret. Fix: restore the RoleBinding for eso-team-a in platform-secrets.",
    resources: tenancy("baseline").concat([dnsPolicy(false),
  ...
  SCENARIOS.push({
    ...
    resources: tenancy("baseline").concat([dnsPolicy(false),
      { kind: "imagevalidatingpolicies", name: "require-signed-images", api: "policies.kyverno.io/v1", cols: ["True", "False", "True", "12d", "Ready"],
  ...
  var TECHNIQUES: Record<string, CnpeGameTechnique> = {
    ...
    "k-yaml": { cmd: "kubectl get {res} -n {ns} -o yaml", about: "the whole object: spec and status, nothing summarized", tool: "kubectl" },
  ...
  /* ── items ─────────────────────────────────────────────────── */
  var ITEMS: Record<string, CnpeGameItem> = {
  ...
  var TILES: Record<string, string> = {".": "grass", ",": "flower", "#": "road", "~": "water", "^": "cliff", "T": "tree", "t": "town", "d": "door", "K": "keep", "G": "gate", "=": "bridge", "s": "sand"};
  var MAP = [
    "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~",
    "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~",
  ...
  function town(sec: string, name: string, dungeon: string, blurb: string, npcs: CnpeGameNpc[]): CnpeGameTown {
    var p = POS[sec];
    return { sec: sec, name: name, blurb: blurb, x: p.x, y: p.y, door: p.door, dungeon: dungeon, npcs: npcs };
  }
  function npc(name: string, lines: string[], teaches?: string): CnpeGameNpc { var n: CnpeGameNpc = { name: name, lines: lines }; if (teaches) n.teaches = teaches; return n; }

  var TOWNS: CnpeGameTown[] = [
    /* ── Substrate Downs: domain 1 ── */
    town("1.1", "Portmouth", "svc-selector", "A harbor town where every road ends at a Service. The wiring here is what everything else stands on.", [
    ...
    town("2.1", "Commitvale", "tekton-trigger", "Every change here arrives as a commit. The valley reconciles, forever, whether you watch or not.", [
      ...
      npc("Watchman Tobe", ["Failure modes to recognize on sight: two controllers on one resource is an infinite war; a manual fix disappears because self-heal did its job; prune deletes production when you delete a file; immutable fields need delete and recreate.", "Stale but green: suspended reconciliation, a lost webhook, a long interval. Everything healthy and nothing current. Always check when it last synced before believing a green dashboard."])
    ]),
    ...
    town("3.1", "Thinvale", "xr-paused", "The thinnest viable town. Vocabulary and judgment live here, and the paper's own words are the answer key.", [
    ...
    town("3.5", "Crossplain", "xp-provider-rbac", "A wide plain where an XRD promises, a Composition fulfills, and an XR is the ten lines a developer types.", [
    ...
    town("4.4", "Spanbridge", "otel-exporter", "A bridge of spans. A trace is only a trace if context propagated, and the collector is where the platform team decides what happens to it.", [
      ...
      npc("Address-keeper Nora", ["The in-cluster OTLP endpoints are <code>otel-collector.tracing.svc:4317</code> for gRPC and 4318 for HTTP. Memorize the form svc.ns.svc:port; every tracing task starts by knowing the address.", "Jaeger's collector listens on the same ports next door. If the destination exists and is idle, the fault is in what points at it."]),
    ...
    town("5.1", "Bindgate", "rbac", "A gate with a ledger of bindings. Every Forbidden error decomposes the same way: subject, verb, resource, namespace.", [
      ...
      npc("Verb-splitter Casimir", ["get, list and watch are separate verbs. Granting get and expecting kubectl get pods to work is a classic mistake. Subresources are their own strings: pods/log, pods/exec, deployments/scale.", "Built-ins: view reads without secrets, edit writes without RBAC, admin manages RBAC in the namespace, cluster-admin is everything. Least privilege says bind the smallest one that works.", "<code>kubectl create rolebinding NAME --clusterrole=view --serviceaccount=NS:SA -n NS</code>: what most fixes in this march come down to."], "f-rb"),
    ...
    town("5.4", "Auditmoor", "image-unsigned", "A moor of ledgers. Who did what to what, what is inside the thing we shipped, and which controls are we failing right now.", [
      ...
      npc("Report-reader Frida", ["Trivy Operator rescans continuously and materializes results as CRs: VulnerabilityReports, SBOMReports, ConfigAuditReports, ClusterComplianceReports. Compliance as queryable API objects rather than PDF attachments.", "kubectl, RBAC, GitOps and dashboards all work on your compliance data for free once it is an object."]),
    ...
    town("5.5", "Cipherford", "eso-store-auth", "A ford where every crossing shows a certificate. Who is this workload, cryptographically? The ServiceAccount is the identity.", [
      npc("Tunnel-keeper Yngve", ["Istio ambient has no sidecars: a per-node ztunnel wraps traffic in mTLS over HBONE using per-workload certificates from istiod. Enrollment is one label on the namespace.", "The identities inside are SPIFFE IDs: <code>spiffe://cluster.local/ns/NS/sa/SA</code>. Note what that is built from: the ServiceAccount is the identity."]),
