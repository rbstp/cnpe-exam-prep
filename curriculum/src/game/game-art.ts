...
(function () {
  ...
  var DOOR_SEALED: Grid = ["................", "....CCCCCCCC....", "...CccccccccC...", "..CccKKKKKKccC..", "..CcKtTtTtTKcC..", "..CcKtTtTtTKcC..", "..CcKKKKKKKKcC..", "..CcKtTyyTTKcC..",
  ...
  function dungeonTile(d: number, column: 0 | 1, row: 0 | 1 | 2, open: boolean) {
    var crypt = cached("crypt." + d + "." + open, function (k) {
      ...
      for (var y = 0; y < 3; y++) for (var x = 0; x < 2; x++) k.drawImage(api.grass((x + y) % 4, d), x * TILE, y * TILE);
      ...
      for (y = 0; y < 15; y++) {
        ...
        if (y % 3 === 0) for (x = inset + 2; x < 29 - inset; x += 5) block(k, roofLight, x, y + 2, 3, 1);
      }
      block(k, light, 12, 1, 8, 1);
      block(k, edge, 1, 17, 29, 2); block(k, roofLight, 1, 17, 21, 1);
      block(k, edge, 3, 40, 26, 3); block(k, light, 3, 40, 19, 1);
      block(k, edge, 22, 22, 4, 8); block(k, s.y, 23, 23, 2, 5);
      block(k, shade, 24, 23, 1, 6);
      var doorSlots: Slots = { c: wall, C: light, K: edge, t: g.t, T: g.T, y: s.y, Y: s.Y,
        v: mix(edge, P!.info, 0.15), V: edge, x: g.x };
      stamp(k, open ? DOOR_OPEN : DOOR_SEALED, doorSlots, 0, TILE);
      block(k, g.r, 0, 29, 4, 3); block(k, light, 3, 30, 12, 1);
      for (y = 18; y < 43; y += 3) {
        block(k, g.o, 27, y, 2, 4);
        block(k, y % 2 ? g.l : g.m, 25 + (y % 3), y + 1, 3, 2);
      }
      [[2, 35], [3, 42], [20, 42], [28, 43]].forEach(function (p) {
        block(k, g.o, p[0], p[1], 3, 2); block(k, g.m, p[0] + 1, p[1] - 1, 2, 2);
      });
    }, TILE * 2, TILE * 3);
  ...
  function sceneColors(d: number): Slots {
  ...
  function townScene(k: CanvasRenderingContext2D, w: number, h: number, d: number, talk: boolean) {
    ...
    if (full) pavedArea(k, center - 6, 0, 13, h, c);
    ...
    for (n = 0; n < people; n++) {
      ...
      if (Math.abs(nx - center) < 17) nx += 19;
    ...
    if (full) {
      ...
      building(k, 30, south, houseW + 5, lowerH, 0, s); building(k, center + 62, south - 1, houseW + 7, lowerH, 1, s);
      for (n = 0; n < 3; n++) {
        scenicTree(k, 126 + n * 24, h - 19, 34 + n % 2 * 7, c, false, false);
        barrel(k, center + 13 + n * 8, street + 31, s);
      }
      pavedArea(k, center - 12, street + 32, 25, 8, c); stamp(k, VILLAGER, folk(s.a), center - 7, street + 23);
    }
      ...
      if (inn) {
        ...
        for (var rowN = 0; rowN < 2; rowN++) for (var shelfN = 0; shelfN < 3; shelfN++) {
          ...
          for (var goods = 0; goods < 5; goods++) {
            ...
            if (rowN) {
              block(k, s.k, gx, by + 4, 5, 10); block(k, color, gx, by + 4, 4, 8);
              block(k, s.w, gx + 1, by + 6, 2, 1); block(k, s.w, gx + 1, by + 10, 2, 1);
            } else {
              block(k, color, gx, by + 7, 5, 6); block(k, s.C, gx + 1, by + 4, 3, 3);
