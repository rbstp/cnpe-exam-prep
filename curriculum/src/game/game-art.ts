...
(function () {
  ...
  function folk(coat: string): Slots { return { k: hair(), p: skin(), a: coat, x: alpha(dark, 0.3) }; }
  ...
  var VILLAGER: Grid = ["................", "......kkkk......", ".....kkkkkk.....", ".....kppppk.....", ".....kpkkpk.....", "......pppp......", ".......pp.......", ".....aaaaaa.....",
  ...
  var api: CnpeArtApi = {
    ...
    bridge: function (vertical) {
      return cached("b" + (vertical ? "v" : "h"), function (k) {
        ...
        var p = P!, s: Slots = { t: p.warnDim, P: lighten(p.warnDim, 0.25), k: darken(p.warnDim, 0.5) };
    ...
    backdrop: function (scene, d, w, h) {
      return cached("bd" + scene + "." + d + "." + w + "x" + h, function (k) {
        ...
        k.fillStyle = mix(p.s1, tintOf(d).c, 0.08); k.fillRect(0, 0, w, h);
        var floor = h - TILE * 2;
        var x: number;
        if (scene === "inn" || scene === "shop") {
          k.fillStyle = darken(p.s2, 0.1); k.fillRect(0, 0, w, floor);            // the wall
          k.fillStyle = darken(p.s3, 0.2); k.fillRect(0, floor, w, h - floor);    // the boards
          k.fillStyle = darken(p.s3, 0.35); for (x = 0; x < w; x += 12) k.fillRect(x, floor, 1, h - floor);
          if (scene === "inn") { stamp(k, BED, s, 24, floor - 24, 2); stamp(k, BED, s, 64, floor - 24, 2); stamp(k, LANTERN, s, 112, floor - 40, 2); stamp(k, VILLAGER, folk(p.warnDim), 160, floor - 28, 2); }
          else { stamp(k, COUNTER, s, 20, floor - 22, 2); stamp(k, VILLAGER, folk(p.viol), 100, floor - 28, 2); stamp(k, LANTERN, s, 150, floor - 40, 2); }
        } else {
          for (x = 0; x < w; x += TILE) { k.drawImage(api.grass((x / TILE) % 4, d), x, floor); k.drawImage(api.grass((x / TILE + 2) % 4, d), x, floor + TILE); }
          k.fillStyle = g.r; k.fillRect(0, floor + 6, w, TILE);                   // the road through the square
          k.fillStyle = g.q; k.fillRect(0, floor + 6, w, 1); k.fillRect(0, floor + 6 + TILE - 1, w, 1);
          stamp(k, HOUSE, s, 8, floor - 30, 2); stamp(k, INN, s, 56, floor - 30, 2); stamp(k, SHOP, s, 104, floor - 30, 2);
          stamp(k, WELL, s, 160, floor - 24, 2); stamp(k, TREE[0], g, 200, floor - 30, 2);
          if (scene === "talk") { stamp(k, VILLAGER, folk(p.accent), 236, floor - 26, 2); stamp(k, VILLAGER, folk(p.bad), 262, floor - 26, 2); }
        }
      }, w, h);
    },
    ...
    check: function () {
      ...
      Object.keys(HERO).forEach(function (f) {
        var fr = HERO[f];
        if (fr.length !== HERO_FRAMES || fr[0].join() === fr[1].join() || fr[0].join() === fr[2].join() || fr[1].join() === fr[2].join()) bad.push("hero-frames-" + f);
      });
