...
(function (root: typeof globalThis & Pick<Window, "CNPE_SYNTAX">) {
  "use strict";
  ...
  function paint(s: string, lang: string): string {
    if (!s) return s;
  ...
  function highlight(html: string, lang: string): string {
    var protectedRe = /(^|\n)([ \t]*#[^\n]*)|('[^'\n]*')|("[^"\n]*")/g;
    var out = "", last = 0, m: RegExpExecArray | null;
