/* CNPE curriculum: the progress merge, and the decisions that go with it.
   ...
   Compiled into assets/merge.js by tools/build-ts.sh. */
(function (root: typeof globalThis & Pick<Window, "CNPE_MERGE" | "CNPE_NAV">) {
  "use strict";

  var BUCKETS: (keyof CnpeTickSets)[] = ["done", "ex", "exam", "exam2"];
  var DAY = 864e5;
  function own(o: object, k: string): boolean { return Object.prototype.hasOwnProperty.call(o, k); }
  function stampOf(v: any): number { var n = +v; return isFinite(n) && n > 0 ? n : 0; }
  ...
  function ownKey(k: string): boolean { return k !== "prototype" && !(k in Object.prototype); }
  ...
  function shiftKey(k: string, by: number): string {   // k ± n days, in local time
    var p = k.split("-");
  ...
  function takeSlots(curV: unknown, incV: unknown): Record<string, number> | null {
  ...
  function dayActs(rec: unknown): number {
    if (!rec || typeof rec !== "object") return 0;
    var r = rec as Record<string, unknown>;
    return countOf(r.c) + countOf(r.x) + countOf(r.s) + countOf(r.e);
  }
  ...
  function streak(store: unknown, today?: string): { streak: number; best: number } {
    var s: Record<string, any> = store && typeof store === "object" ? store : {};
    var days = s.days && typeof s.days === "object" && !Array.isArray(s.days) ? s.days : {};
    var q: Record<string, number> = {};
    ...
    var best = 0, streakRun = 0, prev: string | null = null;
  ...
  /** @param edge oldest day kept */
  function carryRun(s: Record<string, any>, d: string, n: any, edge?: string): void {
  ...
  function seedDays(s: Record<string, any>): void {
    var dm = s.drillmeta;
  ...
  /** @return days dropped */
  function pruneDays(s: any, today?: string): number {
  ...
  function easeOf(rec: Record<string, any>): number {
  ...
  function restOf(rec: any): number {
  ...
  function dueIn(rec: any, now: number): number { return ((rec && +rec.t) || 0) + restOf(rec) * DAY - now; }
  ...
  function obj(v: unknown): Record<string, any> | null { return v && typeof v === "object" && !Array.isArray(v) ? v : null; }
  function gkey(k: string): boolean { return ownKey(k) && GKEY_RE.test(k); }
  /** @return fields that grew */
  function mergeGame(store: Record<string, any>, src: any): number {
    var inc = obj(src && src.game);
    if (!inc) return 0;
    var g = obj(store.game);
    if (!g) g = store.game = {};
    var n = 0, m: Record<string, number> | null;
    ...
    if (iw) {
      ...
      Object.keys(iw).forEach(function (k) {
        ...
        var was = cur || { n: 0 }, next: Record<string, number> = {};
  ...
  function gameHasAnything(g: any): boolean {
    g = obj(g);
    if (!g) return false;
  ...
  function ticks(p: unknown): CnpeTickSets {
    var out: CnpeTickSets = { done: [], ex: [], exam: [], exam2: [] };
  ...
  function sets(b: any): CnpeMergeBase {
    var out = Object.create(null) as any;
    BUCKETS.forEach(function (k) {
      var m = Object.create(null);
      (b && b[k] || []).forEach(function (t: string) { m[t] = 1; });
      out[k] = m;
    });
    return out;
  }
  ...
  function shared(seen: CnpeTickSets, p: unknown): CnpeMergeBase {
    var out = Object.create(null) as any;
    BUCKETS.forEach(function (b) {
  ...
  function canon(v: unknown): string {
    ...
    return "{" + Object.keys(v).sort().map(function (k) {
      return JSON.stringify(k) + ":" + canon((v as Record<string, unknown>)[k]);
    }).join(",") + "}";
  }
  ...
  /** @param b the stored base @param progress the remote copy
      @param rev its revision @param uid the account it came for */
  function pickBase(b: any, progress: unknown, rev: number, uid: string): CnpeMergeBase | null {
  ...
  function hasAnything(p: any): boolean {
    if (!p) return false;
  ...
  function mergeProgress(store: Record<string, any>, src: Record<string, any>, base?: CnpeMergeBase): CnpeMergeCounts {
    ...
    function union(into: Record<string, any>, from: unknown, was: Record<string, 1> | undefined, bucket: "done" | "ex" | "exam"): void {
      ...
      var keys: Record<string, 1> = Object.create(null);
      ...
      Object.keys(keys).forEach(function (k) {
        if (!ownKey(k)) return;
        var r = (from as Record<string, unknown>)[k] ? 1 : 0;
    ...
    (["exam", "exam2"] as const).forEach(function (k) {
